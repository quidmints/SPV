...
| **EIP-170** | `forge test` does **not** enforce the 24,576-byte limit. **`forge build --sizes` does not either, for the contracts that matter most** — measured 2026-08-05: `Core` has **no row in that table at all** (278 rows; `SwapLib`, `Aux`, `BasketLib`, `LevManager`, `LevMath`, `VogueLib` all present). The cause is *not* library linking — `SwapLib`, `Aux`, `LevManager` and `Vogue` are all linked and all appear — and remains **unknown**. **Use `python3 tools/check-contract-sizes.py` instead**: it reads `deployedBytecode.object` from `evm/out/**` for every contract declared in `evm/src`, which is exact (a link placeholder `__$…$__` is 40 hex chars = the 20 bytes its address occupies). **MEASURED MARGINS — RE-MEASURED 2026-08-14. RE-RUN THE SCRIPT; DO NOT TRUST ANY NUMBER WRITTEN HERE: `LevManager` 24,324 (252 left) · `Core` 23,656 (920) · `LevMath` 23,598 (978) · `BTCChannels` 23,528 (1,048) · `Vogue` 23,309 (1,267). Recorded 2026-08-12 as Core 148 / LevManager 224 / LevMath 228 / Vogue 968, and 2026-08-05 as Core 38 / LevManager 70 / LevMath 20 / Vogue 416 — THREE readings, no two alike, because deletions keep returning headroom (the WETH-4626 venue removal alone freed ~770 on `Core` and ~750 on `LevMath`). A stale margin is worse than none: it either blocks an affordable change or waves through an unaffordable one.** ⚠️ **`LevManager` IS THE ONLY BINDING ONE NOW (252 bytes) — it is no longer safe to assume `Core`/`LevMath` are frozen, and equally not safe to assume they are roomy; measure.** This repo has already shipped a `Core` at −126 bytes (undeployable) with a fully green suite; a ~52-byte addition to `Core` on 2026-08-05 consumed more than half the remaining margin before anyone measured it. |
