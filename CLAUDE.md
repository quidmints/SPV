...
| Rust (`quid-ln`) | **Does not build on macOS at all** — `quid-cvm` is Linux-only and transitive. Use Docker `rust:1.90`. **VERIFIED GREEN 2026-08-02: 532 passed / 0 failed.** The invocation matters — this exact command works, and two obvious variants do not:<br>`docker run --rm --sysctl net.ipv6.conf.all.disable_ipv6=0 -v "$PWD":/w -w /w -e CARGO_BUILD_JOBS=1 -e RUSTFLAGS="-C debuginfo=0 -C codegen-units=1" rust:1.90 cargo test --workspace -j 1`<br>• **`--sysctl …disable_ipv6=0` is REQUIRED**: 6 `p2p` tests bind `[::1]:0` and fail `AddrNotAvailable` without it. That is the container, not the code — they pass with it.<br>• **`debuginfo=0` + `-j 1` are REQUIRED** on a default Docker Desktop: its VM is capped near **2 GB** (`docker info` MemTotal — a VM setting, NOT host free RAM), and `rustc` is OOM-killed with **no diagnostic at all** — just *"process didn't exit successfully"*. Raising Docker Desktop → Settings → Resources → Memory to 6–8 GB removes the need. |
