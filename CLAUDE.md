...
| | |
|---|---|
| 🔴 **publicnode IS HEAD-ONLY. A PINNED RUN AGAINST IT IS NOT FLAKY — IT CANNOT ANSWER THE QUESTION** | `evm/.env` says it in its own comment (*"Keyless fallback, head-only but survives sustained load"*) and it is MEASURED: `cast call --block <head−172800>` against `https://ethereum-rpc.publicnode.com` returns **no archive data at all**. ⚠️ **THE SYMPTOM WEARS TRANSPORT'S CLOTHES** — you get `MPP HTTP request … failed: connection reset` or a `database error: failed to get storage for 0x… at <slot>`, never "this node has no archive". Three separate reds today were this. ⇒ **An archive question asked of a non-archive node is a WRONG-ENDPOINT error, not infrastructure noise** — reclassify any RPC-shaped red in a baseline before treating it as a floor. Pinned ⇒ the archive endpoint; head-only work ⇒ publicnode is fine and survives load better. |
| ⚠️ **THE KEEPER FALLS BACK BETWEEN RPC KEYS. FORGE DOES NOT — AND THAT ASYMMETRY MAKES FORK TESTS *LESS* INFORMATIVE AS THE KEEPER GETS MORE ROBUST** | `lev_keeper.rs:1797` reads `ETH_RPC_URL` and falls back to `ANKR_RPC_URL`, so holding a DIFFERENT key in each makes that an operative fallback. **The 9 `vm.envString("ETH_RPC_URL")` sites in Solidity have no such path.** ⇒ when key A degrades, the keeper silently survives while the fork tests fail — **and the failure looks like a code failure.** ▶️ Recovery is swapping the two lines in `evm/.env`. ⛔ Do not read a fork-test red as a contract defect until you have checked which key `ETH_RPC_URL` currently holds; in a shared checkout it can change under a running session. 📌 A multi-block measurement whose endpoint changes MID-RUN must be DISCARDED, and the tell is silent — a faster endpoint does not announce itself, it just stops producing the timeouts that were truncating something. Announce `.env` edits with a timestamp so a run can be checked against them. |
| 🔴 **RUNNING A FULL PINNED SUITE: THROTTLE THE RPC OR LOSE TESTS TO 429s** | A full pinned suite MUST use the archive endpoint (publicnode refuses head−128), and Foundry's default **330 CUPS** drives it into `Max retries exceeded HTTP error 429 … "call rate limit exhausted, retry in 10m0s"`. **Measured 2026-09-07: 4 tests lost to this in one run; 0 after throttling, 1,132 tests confirmed run.** ▶️ **The incantation:** `ETH_RPC_URL=$ANKR_RPC_URL FORK_BLOCK=<n> forge test --rpc-url "$ANKR_RPC_URL" --compute-units-per-second 100 --fork-retries 10 --fork-retry-backoff 1000`. ⛔ **`--rpc-url` IS REQUIRED** — without it forge exits **2** with a usage block and **runs nothing**, which reads as a fast clean run. ⛔ **THESE ARE CLI FLAGS ONLY.** Putting `compute_units_per_second` / `fork_retries` / `fork_retry_backoff` in `foundry.toml` is REJECTED by forge 1.8.1 (`Warning: Found unknown … config`) — I tried it and reverted; three silently-ignored keys that look like protection are worse than none. ⚠️ **A 429 DOES NOT LOOK LIKE A 429 AT THE ASSERTION** — it surfaces as `failed to get storage for 0x… at <slot>`, which reads like a contract bug at a specific address, so you debug the address before reading the rest of the line. |
| ⛔ **DO NOT PIN `FORK_BLOCK` FOR LIVE-ROUTE SUITES** | `ConvertToRouted.t.sol` builds its route from the **live** 1inch API (`tools/fetch_1inch_route.py`) and forks at HEAD by design; its line 24 says so in capitals. Pinning it makes the route's pools no longer match and `got == 0` — **measured: pinned 20 blocks behind head → 0; the same test at head → 100.84 WETH**. I pinned a whole-suite run and manufactured two failures, then "confirmed" them with an A/B **that was also pinned in both arms**. ⇒ 🔴 **AN A/B WITH THE SAME DEFECT IN BOTH ARMS CONFIRMS THE DEFECT, NOT THE HYPOTHESIS.** Run these suites unpinned and quote them separately. |
