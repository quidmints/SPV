import json, math
# Fine-grained QU!D pool LVR vs the spot-tracking full-range baseline.
# Models: pool price = 30-min TWAP (guard pins spot within 50bps of it),
#         concentrated v3 position band +/-2%, REPACK (re-center) on band exit.
# Measures realized IL (=LVR borne by the basket) and the effective K
# (K=0.125 == full-range/no-guard CPMM floor).
...
def run(series, band=0.02, twap_win=6, guard_bps=50, repack_on_exit=True):
    ...
    Q=twap[0]; Pa=Q*(1-band); Pb=Q*(1+band)
    ...
    for i in range(1,n):
        ...
        hold=x*Qn+y
        # rebalance position to Qn within band
        ...
        # repack when price exits the band: realize + re-center at Qn
        if repack_on_exit and (Qn<=Pa or Qn>=Pb):
            repacks+=1
            Pa=Qn*(1-band); Pb=Qn*(1+band)
    ...
    # ---- baseline: full-range CPMM tracking SPOT (no guard, no concentration) ----
    ...
    # full-range CPMM IL over the SPOT path ~ sum r^2/8 (vs hold)
    ...
    # express guarded LVR in K-units (K=0.125 == full-range CPMM on spot)
...
for band in [0.02, 0.01, 0.04]:
    r=run(s, band=band)
    print(f" band=+/-{band:.0%}  ann.vol={r['vol']:.0%}  "
...
def run_nogard(series, band=0.02):
    return run(series, band=band, twap_win=1)   # twap_win=1 -> Q=spot (no smoothing)
print("\n=== guard contribution: TWAP-pinned vs spot-tracking (band +/-2%) ===")
rg=run(s,band=0.02); rs=run_nogard(s,band=0.02)
...
def run1m(series,band=0.02):
    px=[p for _,p in series]; n=len(px)
    tw=[]; W=30
    for i in range(n):
        w=px[max(0,i-W+1):i+1]; tw.append(sum(w)/len(w))
    Q=tw[0];Pa=Q*(1-band);Pb=Q*(1+band);sa=math.sqrt(Pa);sb=math.sqrt(Pb)
    ...
    for i in range(1,n):
        ...
        if il<mdd:mdd=il
        if Qn<=Pa or Qn>=Pb:
            rp+=1;Pa=Qn*(1-band);Pb=Qn*(1+band);sa=math.sqrt(Pa);sb=math.sqrt(Pb)
            tot=x*Qn+y;L=tot/((math.sqrt(Qn)-sa)+Qn*(1.0/math.sqrt(Qn)-1.0/sb));x,y=vfx(L,math.sqrt(Qn),sa,sb)
    lo=min(px);hi=max(px)
    print(f" 1m candles={n}  intraday range {lo:.0f}..{hi:.0f} ({(lo/px[0]-1):.0%}..{(hi/px[0]-1):+.0%})")
