...
# (LVR_t = K*theta*min(r^2, width^2), width-exit capped).
...
# QU!D-accurate knobs (IL-CERT): K=0.71 measured, +/-2% width, certified theta ~0.33,
# venue yield ~6%, fees ~2%, BOLD borrow rate ~7%.
K=0.71; RANGE=0.02; THETA=0.33; Y=0.06; F=0.02; RATE=0.07; S0=0.03
def lvr(i): return K*min(r[i]*r[i], RANGE*RANGE)   # per unit in-width notional, width-capped
...
print(f"days={N}  ann.vol={sigma:.0%}   (K={K}, width={RANGE:.0%}, theta={THETA}, y={Y:.0%}, f={F:.0%}, borrow={RATE:.0%})")
...
# Baseline surplus vs surplus with a leveraged cohort adding (L-1)*LEV_E extra in-width
