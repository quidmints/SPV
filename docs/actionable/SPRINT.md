...
| ~~2~~ | ~~`§T2` terms commitment~~ | ✅ **LANDED `1aaefab3`, AND THE CONSTANTS ARE INDEPENDENTLY DERIVED — not merely "matches the document"** | The deposit address now commits the terms: `PUSH32 sha256(abi.encode(seller, token, minDeliveredUsd)) OP_DROP` in front of the refund leaf. A pure commitment the script drops, so no spending condition changes; substituting any term derives an address the deposit never paid and the settle reverts. **Three loose params folded into `Types.Terms`** — the slop fix and the security fix are the same edit, because the struct IS what gets hashed. 🔑 **THE PROVENANCE PROBLEM, AND HOW IT WAS CLOSED.** I destroyed the original implementation, so the constants survived only in a document *I wrote* — and a test asserting my fresh Solidity equals my own recorded number proves agreement, not correctness. **Re-derived 2026-08-18 from BIP-341 primitives in fresh Python** (tagged hashes, `lift_x`, point add/mul, minimal script-num): `TERMS = 0xa96ad576…86fc` and `Q_with_terms = 0xcd5f8505…66c6`, both matching. ✅ **AND THE CONTROL FIRED — this is what makes the Python trustworthy rather than circular:** the same script with the prefix omitted reproduces `0xd0d16740…e4ca`, **the value this repo pinned for months before §T2 existed and which `TaprootLeafKey.t.sol` independently carries.** A derivation that reproduces a pin it could not have known is correct; the terms value it then produces inherits that. ⇒ **Three-way agreement: fresh Python, fresh Solidity, and the pre-existing pin.** |
