...
| **18** | 🔴 **IS THE CONSENT REGISTRY NEEDED AT ALL? — the question is DELETION, not an intake** (reframed 2026-08-18 after the owner pushed back: *"i thought a registry was not need"*) | 🔴 **DESIGN DECISION, and I nearly shipped the wrong side of it** | **What I found first, and it is still true:** `VaultRegistry::bind_consent` has **zero production callers** — four test assertions and two prose mentions — so the fleet's *"the fleet RELAYS consent"* (`daemon.rs:202`) relays into nothing, and it fails SILENTLY because absence reads as DORMANT rather than error. ⛔ **BUT I BOOKED THE WRONG FIX AND STARTED BUILDING IT** — a `/lp/consent` intake endpoint, written and then reverted unlanded. **The premise is wrong: an intake stores consent for an LP that is ABSENT, and at open the LP cannot be absent.** `vault.rs:244` — the ladder's rungs *"are spends of the 2-of-2 — both require the LP funding half"* — and a MuSig2 spend is an INTERACTIVE session (nonce exchange), so the LP is necessarily live at the moment consent is needed. ⇒ **The registry encodes an absence that cannot occur then**, and the other lane reached this independently: *"if the signer is present at the delivery, the registry is plumbing for an absence that does not exist. Simplification is likely DELETION, not refactor."* Two lanes and the owner, from three directions. ▶️ **The work is therefore: replace store-and-relay with an in-session request at open**, and delete `LpConsent`/`bind_consent`/`consent_for_funding` with it. ⚠️ **THE ONE THING THAT WOULD REFUTE THIS — check it before deleting:** a path that must USE consent while the LP is offline. All five rotation sites now demand a fresh `ExitArming[]` and each spends the 2-of-2, so the LP signs at each; **if any rotation can complete without the LP live, the registry is load-bearing after all.** 📌 It is `1a620c05`'s file — do not delete it out from under that lane. |
