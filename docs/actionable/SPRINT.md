...
| **20** | 🔴🔴 **I BROKE ibiza's WALLET FIXTURE AND ONLY FLAGGED IT IN A CODE COMMENT** (§T2 fallout, booked 2026-08-19) | 🔴 **CROSS-REPO, LIVE, and it fails in the direction that rejects honest users** | §T2 added the terms prefix to the deposit leaf, which moves the taproot output key. `quid-hop`'s `taproot_output_key_matches_the_wallet` says its fixture is *"shared verbatim with `identity-wallet/src/chain/taproot.test.ts`"* — internal `0x02*32`, refund `0xab*32`, height 500000 — and I updated **our** side to `e3d813d7…f8bbe7` while **ibiza still pins `b6df894f…16c65`**. ⇒ **ibiza's QR verifier recomputes a different deposit address than the hop quotes, so it rejects a correctly-paid deposit.** ⚠️ **Nothing compiles the two together**, so no gate catches this — which is why a doc comment in `quid-hop` was not enough and it is booked here. ▶️ The wallet must also learn the prefix itself (`PUSH32 <sha256(seller,token,pricePerBtc,slippageBps)> OP_DROP`), not merely a new constant — the constant is per-fixture, the prefix is the rule. ✅ The old key is retained in our test as the CONTROL: the same script without the prefix still reproduces `b6df894f…`, which is what proves the new value came from adding the prefix rather than from breaking the derivation. |
...
0. **`#19` — DOWNGRADED.** (a) is already built; (b) reduces to one off-chain question (which LN
