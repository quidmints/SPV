...
| **19** | 🔴🔴 **POOL SATS SIT WHERE AN LP CLOSE CAN SWEEP THEM — and the decrement path is still owed** (surfaced by D7's answer; in QUEUE, absent from SPRINT until now) | 🔴 **THE ROOT FIX D7 POINTS AT — it deletes an event, a clamp and a whole question** | D7 established that **nothing bounds the LP's payout from above**, so a close can pay the pool's inventory to the LP and `emit PoolSatsLeftWithLp` is all the chain can do — *"The BTC has already moved — this cannot claw it back."* **That is a detector, not a fix.** Two concrete pieces: **(a)** `14e1d1d5` records that *"a shrink that removes pool sats does not yet decrement the ledger, and an underflow there would brick every close on the channel"* — the `Math.min(booked, totalSats)` at `BTCChannels.sol:624` is the clamp standing in for that missing decrement, i.e. a second guard for the same class of thing (rule 17's own tell); **(b)** the commingling itself — one channel carries the LP's balance AND `poolOwnedSats`. ⇒ **`CLAUDE.md`'s rule-17 WORKED EXAMPLE IS THIS EXACT LEDGER**, and states the fix: *"pool sats may only enter where no LP can claim them"*, which makes `poolOwnedSats`, the clamp and the event **delete themselves**. ⚠️ **Do NOT fix by adding the decrement alone** — that is the third guard on a state that should be unconstructible, and CLAUDE.md's rule 17 says a clamp that survives a root fix was never the fix. 📌 **This also settles the ladder question in `#11`/D7:** the ladder is NOT what protects the pool, so it must not be kept or deleted on that ground — its real job is the attested number the stale-close guard consumes. |
...
0. **`#19` — pool sats sit where an LP close can sweep them.** Now the top item: D7 proved the
   overpayment is undetectable-until-too-late by construction, and rule 17's own worked example names
   the fix. It deletes a ledger, a clamp and an event rather than adding a fourth guard.
