...
| **19** | **POOL SATS IN AN LP CHANNEL** — (a) the decrement ✅ **already built, my booking was stale**; (b) the commingling 🟠 **needs ONE off-chain fact before it is a defect at all** | 🟠 **DOWNGRADED 2026-08-18 on measurement, not argument** | **(a) DONE.** `14e1d1d5` said *"the decrement path is still owed"*; a later commit built it and says so — *"Pool inventory cannot exceed what is left in the channel. This IS the decrement the close-side clamp was standing in for."* Verified by enumeration: `ch.amountSats` has exactly TWO writers (`_applySplice:1421`, `_deliverSwapOut:2272`) and `_releasePoolSats` is called at all three exits — close (`:632`), shrink (`:1481`), delivery (`:2287`). **The `Math.min` clamp is now a belt on a fastened belt, not a stand-in.** ⚠️ **(b) IS PROBABLY NOT A THEFT VECTOR, AND I OVERSTATED IT.** `parkProvenSats` splices the HOP's own BTC in, and an LN splice-in credits the SPLICER's side — so pool inventory should sit on the **hop's** LN balance, where a force-close pays it to the hop and a cooperative close needs the hop's signature to move it. If that holds, `PoolSatsLeftWithLp` is an **accounting-divergence detector**, not a theft detector, and the commingling is safe by LN construction rather than by EVM guard. ▶️ **THE ONE FACT THAT DECIDES IT, and it is off-chain so the EVM cannot see it:** does the LDK splice credit the hop's local balance for the parked amount? **If yes, (b) closes and rule 17 does not apply.** If pool sats can land on the LP's side, (b) is real and the fix is a dedicated inventory channel whose counterparty is not an LP — because the 2-of-2 is exactly what makes the hop's inventory non-rug-able, so it cannot simply move to the hop's wallet. 📌 **Do not act on my earlier framing of this row** — it asserted an unbounded-above payout as a live theft path without checking which side of the channel the sats sit on. |
