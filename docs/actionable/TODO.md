> 📍 **RELOCATED 2026-08-30 (owner direction): this file now lives at `SPV/docs/actionable/TODO.md`.**
> It was `../ibiza/TODO.md`. All the work happens in the SPV repo, so the file follows the work.
> ⚠️ `SPV/CLAUDE.md` pointed at the old path for the §3b LP-signer spec — that pointer is updated in
> the same commit, because a relocated file with a stale pointer is the exact failure this repo keeps
> re-finding (see §ARB-SHARING's dead `REFILL-AND-RESTORATION.md` citation).
...
- [x] `pp/withdraw.ts` — `withdraw(params) → WithdrawOutcome` is the ONE call a screen makes: quotes the relayer per leg (`GET /pp/relay?value=`), prepares with the quoted `feeRecipient`/bps (the plan's `relayFeeBPS` may now be a function of the leg value), proves each leg with the injected prover, relays each; a 402 is answered by re-quoting and re-proving once before anything landed. The relay is not a step the user sees (owner: *"it happens invisibly as part of withdrawal"*). Outcomes: `done | no_relayer | failed{relayed, leg, outcome}`.
- [x] **Backend PROVEN end to end** (2026-09-12): `WithdrawEndToEnd.test_RelayWithRealProof` drives `Entrypoint.relay` with a real Honk proof built for the relayed context (`withdraw_relay_e2e.proof`, generator `--relay`, **3,113,011 gas**); `quid-bridge/tests/pp_relay_e2e.rs` (`PP_RELAY_E2E=1`) dumps that world (`RelayWorldDump`), boots anvil on it, and the Rust handler relays it: quote agrees with the fixture's fee, recipient paid net, `WithdrawalRelayed` emitted, a replay refused as `NullifierAlreadySpent()` with no tx. `RELAY_GAS_ESTIMATE = 3_150_000` prices the quote; re-measure when the verifier changes.
- [ ] The screen: call `withdraw()` from the identity UI (payout amount + `HOP_API.url` + `proveWithdrawal`), render the three outcomes; `no_relayer` is where `buildSelfWithdrawal` is offered, with its privacy cost stated.
...
- [ ] 🔴 **§PQ-PASSPORT-SOUNDNESS — THE PASSPORT RAIL'S ROOT OF TRUST IS CLASSICAL, AND IT IS ICAO'S, NOT OURS** (owner-prompted 2026-09-11: *"dont we have bigger concerns with passports themselves from a post quantum perspective"*). What the circuits verify is ICAO 9303: the DSC's signature over the SOD and the CSCA's over the DSC — ECDSA (P-256, brainpool), RSA-PSS (`§RSAPSS-MSB`), mostly SHA-1 (`evm/src/identity/passport/`, grep-counted: 16 `SHA1`, 12 `ECDSA`, 4 `brainpool`). **A CRQC forges any DSC ⇒ forges any passport's SOD ⇒ unbounded Sybil into `HolderRegistration` and the pool.** The nullifier/commitment layer is hash-based and unaffected; the failure is the ROOT, and no circuit change fixes it — the signatures are the document's. **No PQ profile is deployed in ICAO 9303, and passports are valid ~10 years, so classically-signed documents circulate into the 2040s even after ICAO moves.** ⇒ The levers are POLICY, not crypto: (a) a **registration-epoch cutoff** — a holder root frozen before day-X is not forgeable after it, so pre-CRQC registrations retain their soundness; (b) re-attestation windows; (c) naming the passport rail a classical-era bootstrap with a sunset. ⚠️ Not in `SPRINT.md` `§BTC-4`, which is Bitcoin-side only; this is the identity stack's own quantum item. Decide (a)/(b)/(c) when this scope reopens — it is a design decision, not a build.
- [x] ✅ **§PQ-PROOF-PRIVACY — VERIFIED 2026-09-11: on-chain Honk proofs are NOT a harvest-now-decrypt-later exposure of passport data.** The worry: Honk is BN254/KZG, and if its zero-knowledge were only COMPUTATIONAL (DLog-hiding commitments), every proof already posted on-chain would leak its witness to a future CRQC. **Checked against barretenberg (master `e8e6465`, 2026-09-10; `constants.hpp`, `sumcheck/Sumcheck.md`, `ultra_honk/README.md`): every witness column carries `NUM_MASKED_ROWS = 3` fresh random field elements, sized to hide *"(1) witness commitments, (2) multilinear evaluations of witness polynomials in Sumcheck, (3) evaluations of shifted witness polynomials"* — one random coefficient per leaked linear functional, COUNTING THE COMMITMENT ITSELF as leaked.** A commitment only leaks a functional to an adversary who can invert it (recover τ / DLog) — i.e. the masking budget is sized for exactly the quantum adversary, so hiding is information-theoretic, not DLog-dependent. Same counting in the small-subgroup IPA (`WITNESS_MASKING_TERM_LENGTH = 2`: *"a single random linear term suffices to hide both the commitment [G] and the evaluation G(r)"*). **All 107 checked-in verifiers are the ZK flavor** (`BaseZKHonkVerifier`, `geminiMaskingPoly` + Libra commitments; bb 5.1.0) — the plain UltraHonk flavor, which is NOT zero-knowledge, is used nowhere. ⚠️ Residual, stated not hidden: I read master's constants, and the pinned bb is 5.1.0; the generated verifiers carry the same Gemini-masking + Libra structure, which is the evidence they match. Soundness (forged proofs) is a separate, real, future-tense concern and is the row above, not this one. Cross-ref: `2.18dh`/`2.18dg` — the STARK path (hash-based, PQ-sound) is where a proof-system migration would go.
