...
- **`evm/src/identity/registry/RegistrySourceAnchor.sol`** (new) — anchors a
...
- **`evm/don/notary_registry/main.go`** (new) — a cron-triggered (`cron.Trigger`, daily
  ...
  - **2026-07-24: dropped IPFS entirely, not just left unwired.** The leaf set is no longer routed
    ...
    replayable by any full node. `evm/don/notary_registry/main.go` submits `leaves` directly
...
> has them — verified on an i9-9880H. `evm/noir/codegen-verifiers.sh` is the re-runnable
...
- ✅ **External registry anchoring (CRE): written, NOT yet built/simulated/deployed, but no known
  ...
  `evm/don/notary_registry/main.go` exist; IPFS dependency removed entirely (2026-07-24, see
