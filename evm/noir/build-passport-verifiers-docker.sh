...
docker build --platform linux/amd64 -f "${NOIR_DIR}/passport-verifiers.Dockerfile" -t "${IMAGE}" "${NOIR_DIR}"

printf '%s\n' "${PROFILES[@]}" > "${NOIR_DIR}/.docker-profiles.txt"
trap 'rm -f "${NOIR_DIR}/.docker-profiles.txt"' EXIT
...
docker run --rm --platform linux/amd64 \
  ...
  -v ibiza-swap:/swap \
  -w /repo/evm/noir \
  -e ONLY_FILE=/repo/evm/noir/.docker-profiles.txt \
  ...
  bash -lc '
    ...
    mkdir -p /tmp/bbbin
    ln -sf /repo/evm/noir/node_modules/@aztec/bb.js/build/amd64-linux/bb /tmp/bbbin/bb
