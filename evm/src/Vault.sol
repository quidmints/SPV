...
contract Vault is Ownable, ReentrancyGuard, Shares {
    ...
    // crosses, exiting via `pull`. Geometry + sizing come from the SAME shared
    ...
    function pull(uint id, int percent, address token) external nonReentrant {
        // Body in RangeLib.pull (delegatecall); storage refs mutate in place. §FOLD — this said
        // "BtcLib.pullBtc", which is the copy the merge DELETED: `RangeLib.pull`'s own header
        // records the pair as byte-identical once the storage parameter names were normalised, and
        // `Quid.pull:1208` calls the same body. The comment survived its own subject.
    ...
    ///         this: a BTC boundary order is **USD-funded only** (`outOfRange` reverts
    ...
    ///         `pull` already records that loss for the OWNER-INITIATED close; auto-filling here
