...
library BasketLib {
    ...
    // ═══ §E306 — `BasketLib.qdShareValue` FOLDED IN; `BasketLib.sol` DELETED ═══
    // It was a 29-line file holding ONE `internal` function. An internal library function is
    // INLINED into its caller, so it was never separately deployed — folding it moves zero
    // bytes of bytecode and removes a file and an import. It lives here because this is the
    // basket's valuation library and the function IS the basket-share valuation.
    /// @notice THE ONE VALUATION OF QU!D AS BASKET SHARES (was `ShareMath`'s title).
