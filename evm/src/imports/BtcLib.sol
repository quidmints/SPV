...
library BtcLib {
    ...
    function resize(
        ...
        (a.spotPrice, a.loPrice, a.upPrice,,) = IBandManager(address(this)).repack();
        a.feesPerShare = IBandManager(address(this)).feesPerShare();
        a.usdFees = IBandManager(address(this)).USD_FEES();
    ...
    function requestDeposit(
        ...
        (p.spotPrice, p.loPrice, p.upPrice,,) = IBandManager(address(this)).repack();
        p.feesPerShare = IBandManager(address(this)).feesPerShare();
        p.usdFees = IBandManager(address(this)).USD_FEES();
    ...
    // ═══════════════ §FOLD-BOOK — WAS `LevBookLib`'s VENUE LEGS (2026-08-19) ═══════════════
    // Split by CALLER, which is also the semantic split: every one of these is reached only from
    // `BtcLevManager`. They did NOT go into `LevMath` with the rest of the book — measured, that
    // put `LevMath` at 27,431 bytes, 2,855 OVER EIP-170, and even half of it left ~538 over.
