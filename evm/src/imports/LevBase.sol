...
abstract contract LevBase {
    ...
    /// The range's sync hook (Quid's `syncLev` / Vault's `syncLev`). GOV pin-once, then frozen —
    ///  the SETTER stays per-manager (BtcLevManager fuses it into `init` alongside `venuesFrozen`).
    /// §SLOP — was `rangeSyncHook`, the FOURTH name for one concept (after `V4`, `CORE` and
    ...
    ///         `if (RANGE != address(0)) { try ILevSyncHook(RANGE).syncLev(lp) {} catch {} }`.
    ...
    function _rangePrice() internal view returns (uint px) {
        if (RANGE != address(0)) {
            try ILevSyncHook(RANGE).rangePrice() returns (uint s) { px = s; } catch {}
        }
    }
