...
library VogueLib {
    ...
    function depositETH(
        ...
        if (sent > 0) {
            ...
            uint8 v = venue;
            // EVERY VENUE IS ether.fi (2026-08-07). `supplyVenueBody` routes every kind into the
            // ether.fi adapter, so the six-way dispatch that stood here -- AAVE-v4, Euler, Galaxy,
            // Gauntlet, ether.fi and a five-way SPLIT -- had become six spellings of one call.
            ...
            // A WETH-holding venue must clear that to break even. AAVE v4, measured live: WETH at
            // 1.90% utilisation implies single-basis-point supply APY; weETH at 0.00% utilisation
            // (ZERO borrowed against 714 supplied) yields exactly zero. The suite already knew --
            // routing supply to weETH FIXED testLeverage_LvrControlVsTreatment, the documented
            // baseline failure asserting leverage flow must not leave the passive LP worse off.
            //
            // `v` is accepted-but-ignored so the depositor-facing venue argument and its call sites
            // need not change here; retire it once the VENUE_* constants go.
            v;
