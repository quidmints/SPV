...
interface ISPVGateway {
    ...
    struct BlockData {
        ...
        bytes4 bits;
        uint64 blockHeight;
        // Total chain work UP TO AND INCLUDING this block = parent.cumulativeWork +
        // this block's work. Stored PER-BLOCK (not a single global) so fork-choice
        // compares the true ancestry work of each candidate head. A single global
        // accumulator cancels in the head-vs-head comparison (same constant on both
        // sides) and double-counts on competing-fork boundary submissions, mis-
        // selecting the head across epoch boundaries.
    ...
    /**
     * @notice Checks that the given txId is included in the specified block with a minimum number of confirmations.
     ...
     * @param txIndex_ The index of the transaction in the block's Merkle tree
     * @param minConfirmationsCount_ The minimum number of confirmations required for the block
     * @return True if the txId is present in the block's Merkle tree and the block has at least `minConfirmationsCount_` confirmations, false otherwise
     */
