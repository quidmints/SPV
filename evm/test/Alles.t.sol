...
contract Alles is Test, Fixtures {
    ...
    function testSwapIn_RealLightningHTLC() public {
        ...
        // Bring it up once with: regtest/setup.sh && regtest/setup-ln.sh
        if (keccak256(sig) != keccak256(bytes("READY"))) {
            emit log("regtest/LND harness unavailable (regtest/setup*.sh) - skipping");
            vm.skip(true);
            return;
        }
