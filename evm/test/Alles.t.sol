...
contract AllesFixture is ForkPin, ExitFixture {
    ...
    /// less stack than eleven live locals.
    function _parkSpliceArgs(BTCChannels ch, bytes32 cid, uint seed, uint sats, uint newAmount)
        internal view returns (bytes memory spliceTx, Types.OpenParams memory p)
    {
        string memory j = _spvFixture();
        string memory b = _fixtureKey(seed, sats);
        p = Types.OpenParams({
            fundingBlockHash:   bytes32(uint(0x9A12CE + seed)),
        ...
        spliceTx = abi.encodePacked(
            ...
            hex"01", _le(newAmount, 8), bytes1(uint8(spk.length)), spk,
            hex"00000000");
    }
    ...
    function getAutoManaged(address who) internal view returns (Types.Deposit memory) {
        (uint pooled, uint usd_owed, uint fees_usd) = ETH.autoManaged(who);
...
contract Alles is AllesFixture {
    ...
    function testFeeAccrual() public {
        ...
        // §BTC-10b(c): was `feesPerShare()`, which had no writer — so `assertGe` compared 0 to 0
        // and could not fail. `USD_FEES` is the accumulator swap premiums actually reach.
        uint ethFeesBefore = ETH.USD_FEES();
        ...
        uint ethFeesAfter = ETH.USD_FEES();
    ...
    function testBtcLp_swapInAccruesTheBtcLegFee() public {
        ...
        // §BTC-10b(c) + §MIN-CHARGE-MISSES-THE-SWAP-IN-RAIL — capture the LIVE accumulator.
        uint usdFeesPre = BTC.USD_FEES();
        ...
        //    the present.
        // ⭐ **THE TRIPWIRE ABOVE FIRED, AND THIS IS ITS RESOLUTION — READ THE BLOCK ABOVE FIRST.**
        // It asserted `feesPerShare` did NOT move, deliberately inverted, so that *"the moment
        // anything starts charging on a swap-in — the v4 trading-fee leg returning, or the skew lane
        // being extended to `creditSwapIn`"* the test would fail and someone would come back.
        // **The skew lane WAS extended to `creditSwapIn`** (owner ruling: the swap-IN was a free
        // option against our own TWAP, priced at 0 while the swap-OUT sibling paid 420 ppm), so the
        // BTC-LEG-FEE decision has LANDED and the assertion is now positive rather than inverted.
        // ⚠️ The old block warned that swapping the subject to `USD_FEES` would be *"doubly wrong …
        // `USD_FEES` does not move either"*. **That measurement is now stale: it does move, because
        // `retainFee` credits it.** `feesPerShare` itself is gone — it never had a writer.
        assertGt(BTC.USD_FEES(), usdFeesPre,
            "a swap-in must now charge the LP leg: retainFee -> recordFee -> creditFee -> USD_FEES. "
            "If this is flat the charge was removed from creditSwapInBody and the free option "
            "against our own TWAP is back.");
        (uint pooledAfter,,) = BTC.autoManaged(User01);
