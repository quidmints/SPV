...
contract BtcBandThetaProbe is Alles {
    function test_BtcBand_ThetaThrottlesInRangePairing() public {
        AUX.setBTCChannels(address(this)); // impersonate BTCChannels (registerBtcLp is gated)

        // Baseline: theta fails open (fork avgYield ~ 0 -> derivedThetaWadBtc()==0 -> 1e18 in the
        // clamp) so the whole surplus-permitted slice pairs in-range (grows POOLED_BTC).
        uint p0 = CORE.POOLED_BTC();
        BTC.registerBtcLp(User01, 2e7);            // 0.2 BTC
        uint d1 = CORE.POOLED_BTC() - p0;
        ...
        // value ONLY -- the real addLiqChannel/registerBtcLp clamp path runs unchanged. With
        // theta*vogueAvail << current POOLED_BTC, the in-range headroom collapses to ~0, so the
        ...
        uint p1 = CORE.POOLED_BTC();
        BTC.registerBtcLp(User02, 2e7);            // same sats, theta << 1
        uint d2 = CORE.POOLED_BTC() - p1;
