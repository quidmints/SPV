...
contract LevCascadeProbe is AllesFixture {
    ...
    function test_DerivedBand_GasRebalanceConstantMatchesARealRebalance() public {
        ...
        uint before = gasleft();
        lm.rebalance(lps[0], 0, DEX_WETH_USDC);                  // the borrow leg included -- this is the real cost
        uint used = before - gasleft();

        // MUST TRACK `LevBase.GAS_REBALANCE` (internal, so it cannot be read from here).
        // ⚠️ The bracket is deliberately WIDE: gas-schedule drift and fork cold-access variance are
        // both real, and the miss this test exists to catch was 3x, not 10%.
        assertLe(used, 1_800_000, "a real rebalance costs MORE than GAS_REBALANCE -- raise it in LevBase");
        assertGe(used,   700_000, "GAS_REBALANCE now over-prices gas -- lower it in LevBase");
    ...
    function test_LevFeeLane_EarnsFees_UnwindOnly_SeizureBurnsClean() public {
        ...
        _openAtEntry(lps[0], 5 ether);
        _rallyRange(_entryPrice(lps[0]), 0.2e18, 20, 8_000 * USDC_PRECISION);
        lm.rebalance(lps[0], 0, DEX_WETH_USDC);                                 // real levered position
    ...
    function test_ProtectFromQuid_HostileOperatorNetsZero() public {
        ...
        lm.rebalance(lp, 0, DEX_WETH_USDC); lm.rebalance(lp2, 0, DEX_WETH_USDC);
    ...
    function test_CascadeDelever_CorrelatedCrash() public {
        ...
        for (uint i; i < 3; i++) {
            lm.rebalance(lps[i], 0, DEX_WETH_USDC);
    ...
    function test_Isolation_StuckLpDoesNotTouchAnother() public {
        ...
        _openAtEntry(lps[1], 4 ether);
        _rallyRange(_entryPrice(lps[0]), 0.2e18, 24, 8_000 * USDC_PRECISION);
        lm.rebalance(lps[0], 0, DEX_WETH_USDC); vm.roll(block.number + 1); vm.warp(block.timestamp + 31 minutes);
    ...
    function test_MEV_OracleFloorRejectsSandwich() public {
        ...
        lm.rebalance(lps[0], 1e30, DEX_WETH_USDC);
    }
    ...
    function test_Economic_LeversToProvenIlTarget() public {
        ...
        for (uint k; k < 8; k++) lm.rebalance(lps[0], 0, DEX_WETH_USDC); // keeper loops successive ticks toward target
    ...
    function test_BufferExhaustion_CapBindsAtViolentMove() public {
        ...
        for (uint k; k < 8; k++) { try lm.rebalance(lps[0], 0, DEX_WETH_USDC) {} catch {} }
