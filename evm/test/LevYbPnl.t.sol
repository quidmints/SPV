...
/// @notice PROOF (not a JS sim): does the YB IL-protect actually CANCEL the band LP's impermanent loss?
...
contract LevYbPnlProof is Test {
    ...
    /// √p band LP's ETH after a move to ratio `r` (WAD): E0/√r.
    function _bandEth(uint r) internal pure returns (uint) { return E0 * WAD / _sqrtWad(r); }
    /// The ETH the band sold = the IL (ETH terms): E0·(1 − 1/√r).
    function _ilEth(uint r) internal pure returns (uint) { return E0 - _bandEth(r); }

    function testProof_OnlyDynamicSizingCancelsIL() public {
        uint[4] memory rs = [uint(12e17), 15e17, 2e18, 3e18]; // 1.2x, 1.5x, 2x, 3x
        emit log_string("r | band-only | keeper L=1/a (a=1) | STATIC L=2 (code) | DYNAMIC (re-add sold) | HODL=E0");
        for (uint i = 0; i < rs.length; i++) {
            uint r = rs[i];
            uint band = _bandEth(r);                         // band LP ETH (bears full IL)
            uint il = _ilEth(r);                             // ETH the band sold

            // (a) keeper's coded rule: L = 1/α. weETH is ∝p ⇒ α=1 ⇒ L=1 ⇒ ZERO buffer ⇒ no protection.
            uint keeperAlpha = band + 0;
            ...
            uint staticL2 = band + E0;

            // (c) DYNAMIC: re-add exactly the ETH the band sold ⇒ combined = E0 = HODL ⇒ IL CANCELLED.
            uint dynamicSized = band + il;

            emit log_named_uint("  r (WAD)", r);
            emit log_named_uint("    band-only ETH", band);
            ...
            assertEq(keeperAlpha, band, "keeper L=1/alpha leaves the LP with the full band IL (no protect)");
            assertLt(keeperAlpha, E0, "...i.e. below HODL by the IL");
        }
        emit log_string("VERDICT: IL is cancelled ONLY by sizing the borrow to the MEASURED band-sold ETH.");
    ...
    function testProof_CorrectKeeperTargetLtv() public {
        ...
        for (uint i = 0; i < rs.length; i++) {
            uint r = rs[i];
            uint band = _bandEth(r);
            uint buffer = _ilEth(r);                          // ETH to re-add
            // target LTV = debt/collateral = buffer / (band + buffer) = buffer / E0  (since band+buffer=E0)
            uint targetLtvWad = buffer * WAD / (band + buffer);
    ...
    // we KEEP: capture the rise, re-add exactly the band-sold ETH; below entry a long LP simply holds.
}
