//! Serializable api error types and error kinds returned by various quid
//! services.
...
pub struct ErrorResponse {
    ...
    /// Whether `data` contains sensitive information that Quid shouldn't see
    /// (e.g. a route). Such data may still be logged by the app or in SDKs but
    /// shouldn't be logged inside of Quid infra.
    #[serde(default)] // For backwards compat
    pub sensitive: bool,
}

/// A 'trait alias' defining all the supertraits an API error type must impl
/// to be accepted for use in the `RestClient` and across all Quid APIs.
pub trait ApiError:
    ToHttpStatus
    + From<CommonApiError>
    + From<ErrorResponse>
...
macro_rules! api_error {
    ($api_error:ident, $api_error_kind:ident) => {
        ...
        pub struct $api_error<D = serde_json::Value> {
            ...
            /// Structured data associated with this error.
            pub data: D,
            /// Whether `data` contains sensitive information that Quid
            /// shouldn't see (e.g. a route). Such data may still be logged by
            /// the app or in SDKs but shouldn't be logged inside Quid infra.
            pub sensitive: bool,
        }
...
///   library code (e.g. `RestClient`, `quid_api::server`) which cannot assume a
