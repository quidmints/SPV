...
// Assert that a Quid issuer (e.g. "username@quid.io") fits within limits.
quid_std::const_assert!(
    BoundedString::MAX_CHARS >= USERNAME_MAX_LENGTH + "@quid.io".len()
);
...
impl BoundedString {
    ...
    pub fn truncate(mut s: String) -> Option<Self> {
        quid_std::string::truncate_bytes(&mut s, Self::MAX_BYTES);
        quid_std::string::truncate_chars(&mut s, Self::MAX_CHARS);
