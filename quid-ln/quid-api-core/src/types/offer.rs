...
impl Offer {
    ...
    /// The self-reported payee name, if any. Ex: "satoshi@quid.io", "Satoshi"
    ...
    pub fn preflight_routable_node(
        ...
        for path in self.0.paths() {
            // Need to special case Quid LSP to make smoketests pass where there
...
mod arb {
    ...
    use quid_crypto::rng::{FastRng, RngExt};
    use lightning::{
...
mod test {
    ...
    //
    // `cargo test -p quid-api-core --lib -- offer_dump --ignored --nocapture`
    #[ignore]
    #[test]
    fn offer_dump() {
        ...
        let issuer = Some("philip@quid.io".to_owned());
    ...
    /// ```bash
    /// $ cargo test -p quid-api-core --lib -- offer_decode --ignored --nocapture
    /// ```
    #[ignore]
    #[test]
    fn offer_decode() {
