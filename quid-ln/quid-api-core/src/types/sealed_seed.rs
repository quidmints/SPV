...
///   the [`Network`] supplied by the Quid operators via CLI args.
/// - To encrypt an existing [`RootSeed`] (and [`DeployEnv`] and [`Network`])
///   into a [`SealedSeed`], use [`seal_from_root_seed`].
///
/// See [`quid_enclave::enclave::seal`] for more implementation details.
///
...
/// to validate the [`Network`] that Quid passes in via CLI args, preventing
...
impl SealedSeed {
    ...
    pub fn seal_from_root_seed<R: Crng>(
        ...
        let sealed = sealer
            .seal(random_keyid, Self::LABEL, json_bytes_cow)
    ...
    pub fn unseal_and_validate(
        ...
        let unsealed_json_bytes = sealer
            .unseal(sealed, Self::LABEL)
...
impl ed25519::Signable for SealedSeed {
    const DOMAIN_SEPARATOR: [u8; 32] = array::pad(*b"QUID-REALM::SealedSeed");
}
    ...
    fn test_seal_unseal_roundtrip() {
        ...
        proptest!(|(
            ...
            let sealed_seed = SealedSeed::seal_from_root_seed(
                ...
                measurement,
                machine_id,
                &enclave::NativeSealer,
            )
            .unwrap();

            let (root_seed2, env2, network2) = sealed_seed
                .unseal_and_validate(&measurement, &machine_id, &enclave::NativeSealer)
                .unwrap();
