...
pub async fn run(
    ...
    match std::env::var("QUID_LEV_MANAGER") {
        Ok(lm_str) => {
            ...
            // Block to scan `Vogue.Deposit` logs from for the self-funding compound crank (the Vogue deploy
            // height). Unset ⇒ 0 (genesis: correct but re-scans the whole chain each sweep — set it in prod).
            
            let lp_scan_from: u64 = std::env::var("QUID_VOGUE_DEPLOY_BLOCK")
