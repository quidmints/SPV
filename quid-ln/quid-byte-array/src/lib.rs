...
// Re-export so we can cleanly use `quid_hex` in the
// `impl_fromstr_fromhex!` macro without dependent crates needing a random
// `quid_hex` dep.
#[doc(hidden)]
pub use quid_hex;
use quid_hex::hex::{self, FromHex, HexDisplay};
...
///
/// ```ignore
/// quid_byte_array::impl_fromstr_fromhex!(Measurement);
/// ```
#[macro_export]
macro_rules! impl_fromstr_fromhex {
    ($type:ty, $n:expr) => {
        impl std::str::FromStr for $type {
            type Err = $crate::quid_hex::hex::DecodeError;
            fn from_str(s: &str) -> Result<Self, Self::Err> {
                <Self as ByteArray<$n>>::from_hex(s)
            }
        }
        impl $crate::quid_hex::hex::FromHex for $type {
            fn from_hex(
                s: &str,
            ) -> Result<Self, $crate::quid_hex::hex::DecodeError> {
                <[u8; $n]>::from_hex(s).map(Self::from_array)
            }
        }
    };
...
///
/// ```ignore
/// quid_byte_array::impl_debug_display_hex!(Measurement);
/// ```
#[macro_export]
