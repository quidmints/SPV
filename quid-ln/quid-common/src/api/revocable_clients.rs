...
pub struct RevocableClient {
    ...
    /// The authorization scope this client may exercise. Enforced via
    /// [`Scope::has_permission_for`]: `update` refuses to broaden it (a `NodeConnect`
    /// client can never be escalated to `All`), and it is granted only at creation by
    /// the node owner's full authority.
    pub scope: Scope,
...
impl RevocableClient {
    ...
    pub fn is_expired_at(&self, now: TimestampMs) -> bool {
        if let Some(expiration) = self.expires_at && now > expiration {
            return true;
        }
        false
    }
...
impl RevocableClient {
    ...
    pub fn update(&self, req: UpdateClientRequest) -> anyhow::Result<Self> {
        ...
        if let Some(scope) = req_scope {
            // An update may only NARROW (or keep) a client's scope — never broaden it.
            // `has_permission_for` is true iff the CURRENT scope already permits the
            // requested one, so a connect-only (`NodeConnect`) client can never be
            // escalated to `All` via update. Client creation (owner-authority) is the
            // only place a scope is granted from scratch.
            if !self.scope.has_permission_for(&scope) {
                return Err(anyhow!(
                    "Cannot broaden a client's scope beyond its current grant"
                ));
            }
            out.scope = scope;
        }
...
mod test {
  ...
  "is_revoked": false
    ...
    // Scope enforcement is now EFFECTIVE: an update may narrow or keep a client's scope,
    // but never broaden it (a connect-only client can't be escalated to full authority).
    #[test]
    fn scope_update_cannot_broaden() {
        let pk = *RootSeed::from_u64(2).derive_user_key_pair().public_key();
        let mk = |scope| RevocableClient {
            pubkey: pk,
            created_at: TimestampMs::from_secs_u32(1),
            expires_at: None,
            label: None,
            scope,
            is_revoked: false,
        };
        let upd = |scope| UpdateClientRequest {
            pubkey: pk,
            expires_at: None,
            label: None,
            scope: Some(scope),
        ...
        // A connect-only client CANNOT be escalated to All.
        assert!(
            mk(Scope::NodeConnect).update(upd(Scope::All)).is_err(),
            "must reject broadening NodeConnect -> All"
        );
        // Keeping the same scope is allowed.
        assert!(mk(Scope::NodeConnect).update(upd(Scope::NodeConnect)).is_ok());
        // An All client MAY be narrowed to NodeConnect.
        let narrowed = mk(Scope::All)
            .update(upd(Scope::NodeConnect))
            .expect("narrowing All -> NodeConnect is allowed");
        assert_eq!(narrowed.scope, Scope::NodeConnect);
    }
