...
use quid_crypto::ed25519::{self, Signable};
...
/// A Quid user, as represented in the DB.
...
/// A Quid user's primary identifier, derived from the root seed.
...
impl UserPk {
    ...
    pub const fn from_ref(inner: &[u8; 32]) -> &Self {
        quid_std::const_utils::const_ref_cast(inner)
    }
...
quid_byte_array::impl_byte_array!(UserPk, 32);
quid_byte_array::impl_fromstr_fromhex!(UserPk, 32);
quid_byte_array::impl_debug_display_as_hex!(UserPk);
...
quid_byte_array::impl_byte_array!(ShortUserPk, 4);
quid_byte_array::impl_fromstr_fromhex!(ShortUserPk, 4);
quid_byte_array::impl_debug_display_as_hex!(ShortUserPk);
...
impl Signable for NodePkProof {
    const DOMAIN_SEPARATOR: [u8; 32] = array::pad(*b"QUID-REALM::NodePkProof");
}
