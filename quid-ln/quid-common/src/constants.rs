...
use quid_std::{const_assert, const_concat_str};
...
///
/// 1) having a sufficient number of recent versions approved so that Quid has
///    the ability to downgrade users (by yanking versions) if it is discovered
///    that a node release is broken in some way, and
/// 2) having so many versions approved that Quid could downgrade users to an
...
/// The amount of liquidity (in sats) that Quid supplies to us for free, which
/// we are not expected to pay interest on. This is also the amount of liquidity
/// Quid's LSP will supply to a user in their first JIT zeroconf channel open.
// 50k sats = 0.0005 BTC = $25 at $50k/BTC or $50 at $100k/BTC
pub const FREE_LIQUIDITY_SAT: u32 = 50_000;

/// The maximum amount of liquidity that Quid will supply to a user in one tx.
...
/// Provision mode uses "{mr_short}.provision.quid.io" and run mode uses
/// "run.quid.io". These DNS names don't actually resolve.
pub const NODE_RUN_DNS: &str = "run.quid.io";
...
pub const NODE_PROVISION_DNS_SUFFIX: &str = ".provision.quid.io";

// --- Esplora --- //
// Quickly test these by appending /fee-estimates and opening in browser,
// e.g. "https://testnet.ltbl.io/api/fee-estimates"

pub const MAINNET_QUID_MEMPOOL_ESPLORA: &str = "https://quid.mempool.space/api";
// Introduced in node-v0.6.8, lsp-v0.6.28
pub const MAINNET_QUID_BLOCKSTREAM_ESPLORA: &str =
...
pub const MAINNET_ESPLORA_WHITELIST: [&str; 3] = [
    MAINNET_QUID_MEMPOOL_ESPLORA,
    MAINNET_QUID_BLOCKSTREAM_ESPLORA,
    MAINNET_PUBLIC_BLOCKSTREAM_ESPLORA,
];

// Introduced in node-v0.7.12
pub const TESTNET3_QUID_MEMPOOL_ESPLORA: &str =
    "https://quid.mempool.space/testnet/api";
// Introduced in node-v0.6.8, lsp-v0.6.28
// NOTE: our ipwl doesn't currently work for testnet3
pub const TESTNET3_QUID_BLOCKSTREAM_ESPLORA: &str =
...
pub const TESTNET3_QUID_ESPLORA: &str = "https://esplora.staging.quid.io/api";
pub const TESTNET3_ESPLORA_WHITELIST: [&str; 5] = [
    TESTNET3_QUID_BLOCKSTREAM_ESPLORA,
    TESTNET3_PUBLIC_BLOCKSTREAM_ESPLORA,
    TESTNET3_QUID_ESPLORA,
    TESTNET3_QUID_MEMPOOL_ESPLORA,
...
// websites that we make requests to, including Quid itself. For security, we
...
// TLS config whenever we make a request to Quid or an external site. For
...
// - Save it to "quid-common/data/<root_ca_name>_root-ca-cert.der"
...
// NOTE: There is NO QU!D CA of any kind (prod, staging, or dummy). The gateway is outer transport only — the
// node behind it is verified end-to-end by SGX remote attestation — so it presents an ordinary PUBLIC cert
// (auto-issued + auto-renewed by Let's Encrypt), trusted via the standard WebPKI roots in `quid_server_verifier`
// in every environment. Nothing to generate, ship, or store.

/// Google Trust Services Root R1 (RSA), used by `googleapis.com` and
/// `dns.google`.
// `curl https://i.pki.goog/r1.crt -o quid-common/data/google-trust-services-root-r1-ca-cert.der`
...
/// `dns.google`.
// `curl https://i.pki.goog/r2.crt -o quid-common/data/google-trust-services-root-r2-ca-cert.der`
...
/// `dns.google`.
// `curl https://i.pki.goog/r3.crt -o quid-common/data/google-trust-services-root-r3-ca-cert.der`
...
/// `dns.google`.
// `curl https://i.pki.goog/r4.crt -o quid-common/data/google-trust-services-root-r4-ca-cert.der`
...
// `curl https://i.pki.goog/gsr4.crt -o quid-common/data/globalsign-root-r4-ca-cert.der`
