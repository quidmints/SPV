...
impl DeployEnv {
    ...
    // NOTE: the old `validate_sgx(use_sgx)` blanket "staging/prod ⇒ SGX" check was
    // retired in favour of the backend/role attestation policy in
    // `quid_enclave::backend::require_backend_for_role` (called from
    // `quid_hop::seed::load_or_provision_from_env`): a serves-others role needs a
    // custody-ready TEE, while an individual self-host may run on any backend.
    ...
    pub fn gateway_url(
        ...
        match self {
            ...
            Self::Staging => Cow::Borrowed("https://gateway.staging.quid.io"),
            Self::Prod => Cow::Borrowed("https://gateway.quid.io"),
        }
    }
