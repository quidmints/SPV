...
/// Basically `bdk::Balance`, so that `quid-common` doesn't need to depend on
/// `bdk`.
///
...
pub struct LightningBalance {
    ...
    /// to account for an estimate of the total fees paid to Quid's LSP and the
    ...
    /// usable channels, then adjusted to account for the minimum fees Quid's
    /// LSP could charge us.
    ///
    ...
    ///   such as paying another Quid user.
