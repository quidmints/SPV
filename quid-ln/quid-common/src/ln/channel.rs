...
quid_byte_array::impl_byte_array!(LxChannelId, 32);
quid_byte_array::impl_fromstr_fromhex!(LxChannelId, 32);
quid_byte_array::impl_debug_display_as_hex!(LxChannelId);
...
quid_byte_array::impl_byte_array!(LxUserChannelId, 16);
quid_byte_array::impl_fromstr_fromhex!(LxUserChannelId, 16);
quid_byte_array::impl_debug_display_as_hex!(LxUserChannelId);
...
/// traits, flattens nested structure, and contains only the fields Quid needs.
...
pub struct LxChannelDetails {
    ...
    /// counterparty is a Quid user or Quid's LSP, this is determined by their
    /// [`ChannelHandshakeConfig::our_htlc_minimum_msat`](lightning::util::config::ChannelHandshakeConfig).
    pub outbound_htlc_minimum: Option<Amount>,
    /// The largest outbound HTLC our counterparty will accept. Assuming the
    /// counterparty is a Quid user or Quid's LSP, this appears to be bounded
...
impl LxChannelDetails {
    ...
    pub fn from_ldk(
        ...
        let ChannelDetails {
            ...
            counterparty_shutdown_scriptpubkey: _,
        } = details;
