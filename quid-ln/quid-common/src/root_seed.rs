...
impl RootSeed {
    ...
    const HKDF_SALT: [u8; 32] = array::pad(*b"QUID-REALM::RootSeed");
    ...
    ///
    /// New Quid wallets created > node-v0.9.1 use this to derive their
    /// on-chain wallet BIP32 master xprivs.
    ///
    /// Old Quid on-chain wallets use the [`Self::derive_legacy_master_xprv`]
    /// instead.
    ...
    /// seed.
    ///
    /// This is used for new Quid on-chain wallets created > node-v0.9.1.
    ...
    /// entropy. This makes Quid's old on-chain addresses incompatible with
    ...
    pub fn derive_ldk_seed(&self) -> Secret<[u8; 32]> {
        ...
        let master_xprv = self.derive_legacy_master_xprv(Network::Mainnet);
    ...
    /// Derive this node's Ethereum / EVM secp256k1 signing key.
    ///
    ...
    /// the (untrusted) operator.
    ///
    /// Derivation: a domain-separated 32-byte HKDF secret run through BIP32
    /// `new_master` (HMAC-SHA512 → reduce mod n), identical in shape to
    /// [`Self::derive_node_key_pair`]. This always yields a valid secp256k1 scalar
    /// (no rejection sampling, no panic path), while the HKDF label gives domain
    /// separation from every other derived key.
    pub fn derive_eth_wallet_key(&self) -> secp256k1::SecretKey {
        let seed = self.derive(&[b"ethereum wallet key"]);
        let xprv = bip32::Xpriv::new_master(
            bitcoin::Network::Bitcoin,
            seed.expose_secret(),
        )
        .expect("32-byte seed is always a valid BIP32 master input");
        xprv.private_key
    }
    ...
    ///
    /// [`MIN_PASSWORD_LENGTH`]: quid_crypto::password::MIN_PASSWORD_LENGTH
    /// [`MAX_PASSWORD_LENGTH`]: quid_crypto::password::MAX_PASSWORD_LENGTH
...
mod test {
    ...
    fn when_does_network_matter() {
        proptest!(|(
            ...
            network1 in any::<Network>(),
            network2 in any::<Network>(),
        )| {
    ...
    fn derive_snapshots() {
        ...
        let bip39_master_xpriv =
            seed.derive_bip32_master_xprv(Network::Mainnet);
        assert_eq!(
        ...
        let bip39_testnet_xpriv =
            seed.derive_bip32_master_xprv(Network::Testnet3);
        assert_eq!(
        ...
        // Legacy Quid master xpriv (used for existing on-chain wallets)
        let master_xpriv = seed.derive_legacy_master_xprv(Network::Mainnet);
        assert_eq!(
        ...
        // Legacy Quid master xpriv (Testnet)
        let master_xpriv_testnet =
            seed.derive_legacy_master_xprv(Network::Testnet3);
        assert_eq!(
    ...
    /// Snapshot the enclave-derived EVM signing key so the derivation (and thus
    /// the on-chain-registered hop/LP address) can never silently change out from
    /// under a deployed enclave. Kept as its own test (not folded into
    /// `derive_snapshots`) so it is independent of the other snapshots.
    #[test]
    fn derive_eth_wallet_key_snapshot() {
        let seed = RootSeed::from_u64(20240506);
        let eth_key = seed.derive_eth_wallet_key();
        assert_eq!(
            hex::encode(&eth_key.secret_bytes()),
            "f647775eb556c4ea1b129187d92d06cbfa74c98ed6a5936198975e111089daec",
        );
        // Deterministic: same seed -> same key.
        assert_eq!(
            eth_key.secret_bytes(),
            RootSeed::from_u64(20240506).derive_eth_wallet_key().secret_bytes(),
        );
        // Domain-separated: different from the LDK node key material.
        assert_ne!(
            &eth_key.secret_bytes(),
            seed.derive_ldk_seed().expose_secret(),
        );
    }
    ...
    // $ nix shell .#secretctl
    // $ PASSWORD=".." IN_PATH=".." \
    //     cargo test -p quid-common --lib -- test_decrypt_root_seed --nocapture --ignored
    // ```
    #[test]
    #[ignore]
