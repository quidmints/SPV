...
//!         ikm=master-key,
//!         salt=array::pad::<32>("QUID-REALM::AesMasterKey"),
//!         info=key-id,
//!         out-len=32 bytes,
//!    )
...
impl AesMasterKey {
    const HKDF_SALT: [u8; 32] = array::pad(*b"QUID-REALM::AesMasterKey");
...
impl KeyId {
    ...
    const fn from_ref(arr: &[u8; 32]) -> &Self {
        quid_std::const_utils::const_ref_cast(arr)
    }
...
// See `quid_common::root_seed::RootSeed::derive_vfs_master_key`
