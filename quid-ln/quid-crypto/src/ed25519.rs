...
pub trait Signable {
    ...
    /// `array::pad(*b"QUID-REALM::RootSeed")`, used in the `RootSeed`.
...
quid_std::const_assert_usize_eq!(PKCS_LEN, 83);
quid_std::const_assert_usize_eq!(PKCS_LEN_BAD, 85);
...
mod test {
    ...
    impl Signable for SignableBytes {
        const DOMAIN_SEPARATOR: [u8; 32] =
            array::pad(*b"QUID-REALM::SignableBytes");
    }
    ...
    // ```bash
    // $ cargo test -p quid-common --lib -- pkcs8_der_snapshot_data --nocapture --ignored
    // ```
    #[ignore]
    #[test]
    ...
    fn test_sign_verify_struct() {
        ...
        impl Signable for Foo {
            const DOMAIN_SEPARATOR: [u8; 32] = array::pad(*b"QUID-REALM::Foo");
        }
        ...
        impl Signable for Bar {
            const DOMAIN_SEPARATOR: [u8; 32] = array::pad(*b"QUID-REALM::Bar");
        }
