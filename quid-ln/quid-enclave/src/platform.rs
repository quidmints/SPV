...
#[cfg(not(target_env = "sgx"))]
use quid_hex::hex;
use quid_std::array::{self, ArrayExt};
use ring::{
    aead::{
        ...
        cfg_if! {
            if #[cfg(target_env = "sgx")] {
                ...
                sgx_isa::Report {
                    ...
                    miscselect: miscselect::QUID_FLAGS,
                    _reserved1: [0; 28],
                    attributes: sgx_isa::Attributes {
                        // Just use prod value since the flags are fake anyway
                        flags: attributes::QUID_FLAGS_PROD,
                        xfrm: xfrm::QUID_FLAGS,
                    },
                    ...
                    isvsvn: 0u16,
                    _reserved4: [0; 60],
                    // This is newtyped in `quid_tls_attest_server::quote`
                    reportdata: [0; 64],
...
pub fn machine_id() -> MachineId {
    cfg_if! {
        if #[cfg(target_env = "sgx")] {
            ...
            // bind the signer (quid) not the enclave. this way all our
...
const MACHINE_ID_KEY_ID: [u8; 32] = *b"~~~~ QUID MACHINE ID KEY ID ~~~~";

// --- impl LxKeyRequest and MockKeyRequest --- //

quid_std::const_assert_usize_eq!(
...
impl LxKeyRequest {
    ...
    fn new_sealing_request(random_keyid: [u8; 32]) -> Self {
        let attribute_mask: u64 = attributes::QUID_MASK.bits();
        let xfrm_mask: u64 = xfrm::QUID_MASK;
        let misc_mask: u32 = miscselect::QUID_MASK.bits();
    ...
    fn try_from_bytes(bytes: &[u8]) -> Result<Self, Error> {
        ...
        // Pin the policy: a deserialized keyrequest (e.g. from a substituted
        // Sealed blob) must match our FIXED seal-time sealing policy — only the
        // random `keyid` may vary. Without this, a tampered blob could request a
        // key bound to WEAKER attributes than seal time intended (quid audit
        // LOW-1, defense-in-depth; AEAD already fails closed on a wrong key, but
        // pinning is cheap and removes the widening surface).
        kr.validate_policy()?;
        Ok(kr)
    }

    /// Reject a keyrequest whose policy fields differ from `new_sealing_request`'s
    /// fixed policy. Reconstructs the expected request with the SAME `keyid` so
    /// only the policy (keyname / keypolicy / isvsvn / cpusvn / attributemask /
    /// miscmask) is compared — `keyid` is the only field allowed to vary.
    fn validate_policy(&self) -> Result<(), Error> {
        let expected = Self::new_sealing_request(self.0.keyid);
        let (k, e) = (&self.0, &expected.0);
        let matches = k.keyname == e.keyname
            && k.keypolicy == e.keypolicy
            && k.isvsvn == e.isvsvn
            && k.cpusvn == e.cpusvn
            && k.attributemask == e.attributemask
            && k.miscmask == e.miscmask;
        if matches {
            Ok(())
        } else {
            Err(Error::InvalidKeyRequestPolicy)
        }
    }
    ...
    fn derive_key_material(&self) -> Result<[u8; 16], Error> {
        cfg_if! {
            if #[cfg(target_env = "sgx")] {
                ...
                // `LxKeyRequest` wraps the real `EGETKEY` and is only ever
                // constructed inside SGX; off-SGX the seal path uses
                // `MockKeyRequest` instead, so this is never reached.
                unreachable!("LxKeyRequest is SGX-only; off-SGX uses MockKeyRequest")
            }
        }
    }
    ...
    fn derive_aesgcm_key(key_material: &[u8; 16], label: &[u8]) -> UnboundKey {
        ...
        const HKDF_SALT: [u8; 32] = array::pad(*b"QUID-REALM::SgxSealing");
        UnboundKey::from(
