...
pub enum Error {
    ...
    InvalidKeyRequestPolicy,
    UnsealDecryptionError,
    DeserializationError,
    /// The SEV-SNP firmware-derived seal key could not be obtained (not an
    /// SEV-SNP guest, or the ioctl failed). Sealing FAILS CLOSED rather than
...
/// derive the same machine id, we'll use the enclave signer (Quid) in our key
...
impl fmt::Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let s = match self {
            ...
            Self::InvalidKeyRequestPolicy =>
                "keyrequest policy does not match the seal-time sealing policy",
            ...
            Self::SevKeyUnavailable =>
                "SEV-SNP seal key unavailable (not an SEV-SNP guest or ioctl failed)",
        };
        match self {
...
impl Measurement {
    pub const MOCK_ENCLAVE: Self =
        Self::new(*b"~~~~~~~ QUID MOCK ENCLAVE ~~~~~~");
    pub const MOCK_SIGNER: Self =
        Self::new(*b"======= QUID MOCK SIGNER =======");

    /// The enclave signer measurement (MRSIGNER) our debug enclaves are signed
    /// with — the MRSIGNER of `quid-sgxs-sign/data/dev-sgxs-signer.der`, the key
    /// `run-sgx .. --debug` auto-signs with (via `KeyPair::dev_signer()`).
    /// Regenerate both together with `cargo run -p quid-sgxs-sign --bin
    /// gen-signer -- quid-sgxs-sign/data/dev-sgxs-signer.der`.
    pub const DEV_SIGNER: Self = Self::new(hex::decode_const(
        b"26f048b1fb0ea5cbbb26124b012be52ee72394abaa376392e51bafce397b41fd",
    ));

    /// The enclave signer measurement (MRSIGNER) our production enclaves should
    /// be signed with. Inside an enclave, get the signer with `signer`.
    ///
    /// PLACEHOLDER — generated by `quid-sgxs-sign`'s `gen-signer` to establish
    /// the process; the corresponding private key is NOT in the repo. Before
    /// mainnet, generate the real prod key on an offline / HSM-backed machine
    /// (keep the DER out of the repo) and replace this MRSIGNER with its value.
    pub const PROD_SIGNER: Self = Self::new(hex::decode_const(
        b"ed84f711b2250b45cc816f1770f9ddc54c36bc37d84349826134b67b435112a8",
    ));

    /// The committed placeholder value of [`PROD_SIGNER`] above — there is no real
    /// prod signing key for it. A staging/prod boot must refuse to run while
    /// `PROD_SIGNER` still equals this (enforced by quid-hop
    /// `migration::guard_prod_trust_anchors`). Once `PROD_SIGNER` is replaced with
    /// the real offline/HSM-generated MRSIGNER, this frozen snapshot no longer
    /// matches and the guard passes.
    pub const PROD_SIGNER_PLACEHOLDER: Self = Self::new(hex::decode_const(
        b"ed84f711b2250b45cc816f1770f9ddc54c36bc37d84349826134b67b435112a8",
    ));
    ...
    /// The raw 32-byte measurement (MRENCLAVE), e.g. for the `bytes32 measurement`
    /// field of the EIP-712 `MigrationAuth` operators sign.
...
quid_byte_array::impl_byte_array!(Measurement, 32);
quid_byte_array::impl_fromstr_fromhex!(Measurement, 32);
quid_byte_array::impl_debug_display_as_hex!(Measurement);
...
quid_byte_array::impl_byte_array!(MrShort, 4);
quid_byte_array::impl_fromstr_fromhex!(MrShort, 4);
quid_byte_array::impl_debug_display_as_hex!(MrShort);
...
quid_byte_array::impl_byte_array!(MachineId, 16);
quid_byte_array::impl_fromstr_fromhex!(MachineId, 16);
quid_byte_array::impl_debug_display_as_hex!(MachineId);
...
pub mod attributes {
    ...
    pub const QUID_FLAGS_PROD: AttributesFlags = AttributesFlags::MODE64BIT;

    pub const QUID_FLAGS_DEBUG: AttributesFlags =
        QUID_FLAGS_PROD.union(AttributesFlags::DEBUG);

    /// The flags we want to bind (whether on or off).
    pub const QUID_MASK: AttributesFlags = AttributesFlags::INIT
...
pub mod xfrm {
    ...
    /// Features required by QUID enclaves.
    ///
    ...
    /// recent hardware.
    pub const QUID_FLAGS: u64 = AVX512 | LEGACY;

    /// Require QUID features but allow new ones, determined at runtime.
    pub const QUID_MASK: u64 = QUID_FLAGS;
}
...
pub mod miscselect {
    ...
    pub const QUID_FLAGS: Miscselect = Miscselect::empty();

    /// Bind nothing.
    pub const QUID_MASK: Miscselect = Miscselect::empty();
}
