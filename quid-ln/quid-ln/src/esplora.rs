...
pub fn url_is_whitelisted(esplora_url: &str, network: Network) -> bool {
    match network {
        ...
        // No curated whitelist for these networks ⇒ no url is whitelisted.
        Network::Testnet4 | Network::Signet => false,
...
const MIN_FEERATE_SATS_PER_VBYTE: f64 = 1.0;
const MAX_FEERATE_SATS_PER_VBYTE: f64 = 10_000.0;
...
impl FeeEstimates {
    ...
    fn convert_estimates(estimates: HashMap<u16, f64>) -> BTreeMap<u16, f64> {
        // The esplora backend is only SEMI-trusted: a compromised/misbehaving
        // provider could poison feerates so LDK burns funds on force-close /
        ...
        estimates
            .into_iter()
            .filter_map(|(target, feerate)| {
                if !feerate.is_finite() {
                    return None; // NaN/±inf → drop; lookup falls back + floor applies
                }
                Some((
                    target,
                    feerate.clamp(MIN_FEERATE_SATS_PER_VBYTE, MAX_FEERATE_SATS_PER_VBYTE),
                ))
            })
            .collect()
    }
...
impl Esplora {
    /// Try initializing a [`Esplora`] from *any* of the given Esplora urls,
    ...
    ) -> anyhow::Result<(Arc<Self>, Arc<FeeEstimates>, LxTask<()>, String)>
    {
        ...
        let joined = err_msgs.join("; ");
        Err(anyhow!("Esplora::init_any failed: [{joined}]"))
    }

    /// Initialize a [`Esplora`] client.
    ...
    //
    // [`Esplora::init_any`] relies on (2) to gracefully recover from 'bad'
    ...
    pub async fn init(
        ...
        let tls_config = rustls::ClientConfig::builder_with_protocol_versions(
            quid_tls_core::QUID_TLS_PROTOCOL_VERSIONS,
        )
        .with_root_certificates(quid_tls_core::WEBPKI_ROOT_CERTS.clone())
        .with_no_client_auth();

        // Esplora wraps AsyncClient which in turn wraps reqwest::Client.
        let reqwest_client = {
    ...
    /// Create a dummy Esplora for testing.
    #[cfg(any(test, feature = "test-utils"))]
    pub fn dummy() -> Arc<Self> {
...
mod fee_clamp_tests {
    ...
    #[test]
    fn convert_estimates_validates_and_clamps() {
        let raw = HashMap::from([
            (1u16, 50.0),           // normal → passthrough
            (3u16, 1e9),            // absurd huge → clamped to MAX
        ...
        assert_eq!(got[&3], MAX_FEERATE_SATS_PER_VBYTE, "huge clamped to MAX (no fund burn)");
