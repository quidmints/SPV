...
use quid_api::vfs::{self, Vfs, VfsFile, VfsFileId};
#[cfg(test)]
...
use crate::{
    ...
    traits::{QuidChannelManager, QuidEventHandler, QuidPersister},
...
pub fn spawn_event_replayer_task(handler: impl QuidEventHandler) -> LxTask<()> {
...
/// All event handler methods needed in shared Quid LN logic.
pub trait EventHandlerMethods: Clone + Send + Sync + 'static {
    // --- Required methods --- //
            ...
            LxTask::spawn(event_id.to_string(), {
                ...
                async move {
                    match myself.handle_event(&event_id, event).await {
                        Ok(()) => {
                            info!(
                                "Successfully handled event; \
                                 removing from Quid event queue."
                            );
                        ...
                        // event is managed via Quid's own event queue. So long
    ...
    fn replay_event(
        ...
        async move {
            match self.handle_event(&event_id, event).await {
                ...
                // already in Quid's event queue. If the handling fails, we'll
...
where
    ...
    PS: QuidPersister,
{
    // IDEMPOTENCY (audited): handling is idempotent via the *reactive* path
    // below, so a redundant proactive "are these already spent?" esplora probe
    // is deliberately omitted. If a replay re-runs this for outputs a prior
    // sweep already spent, `spend_spendable_outputs` still builds a tx but its
    // broadcast fails with spent/missing inputs (see `is_spent_or_missing_inputs`
    // branch) → we persist to `unswept_outputs-*` and DISCARD (one-shot, no
    // infinite replay, no fund loss since "spent" means already swept). Transient
    // broadcast failures take the `Replay` branch and retry. `get_internal_address`
    // returns the next *unused* spk, reused across failed replays, so no internal
    // address is leaked. A proactive pre-check would only save one doomed build+
    // broadcast in the already-swept case — not worth plumbing esplora through
    // this signature + the event ctx for.
...
mod test {
    ...
    /// cargo test -p quid-ln -- event_id_basic --show-output
    #[test]
    fn event_id_example() {
        let event_id1 =
