...
use quid_common::{ln::addr::LxSocketAddress, rng::SysRngDerefHack};
#[cfg(doc)]
...
/// The Quid [`MessageRouter`] impl for both the Quid LSP and our user nodes.
...
pub struct QuidMessageRouter {
    network_graph: Arc<NetworkGraphType>,
    kind: Kind,
}

enum Kind {
    /// User nodes are not public nodes and only peer with the Quid LSP. When
...
impl QuidMessageRouter {
    /// Create a new [`QuidMessageRouter`] for user nodes.
    ...
    /// Create a new [`QuidMessageRouter`] for LSP.
...
impl MessageRouter for QuidMessageRouter {
...
mod tests {
    ...
    use quid_api::cli::LspInfo;
    use quid_common::{
        api::user::NodePk, ln::addr::LxSocketAddress, ppm,
        rng::FastRngDerefHack, secp256k1_ctx::SECP256K1,
    };
    use lightning::{
    ...
    fn test_find_path_user_node() {
        ...
        let router = QuidMessageRouter::new_user_node(network_graph, lsp_info);
    ...
    fn test_find_path_lsp() {
        ...
        let router = QuidMessageRouter::new_lsp(network_graph.clone());
        ...
        // intro_node).
        let user_router = QuidMessageRouter::new_user_node(
            network_graph.clone(),
            lsp_info.clone(),
        );
        let nonce =
