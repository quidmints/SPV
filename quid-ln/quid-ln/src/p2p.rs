...
mod test {
    ...
    //
    // ```
    // $ cargo test -p quid-ln --lib --no-run --release
    //     Finished `release` profile [optimized] target(s) in 0.55s
    //   Executable unittests src/lib.rs (target/release/deps/quid_ln-e762b03c4651e07d)
    //
    // $ RUST_LOG=trace RUST_BACKTRACE=1 TEST_ECHO_ITERS=100000 target/release/deps/quid_ln-e762b03c4651e07d \
    ...
    impl PeerManagerTrait for Arc<Mutex<EchoPeerManager>> {
        fn is_connected(&self, _node_pk: &NodePk) -> bool {
            unreachable!("unused by p2p tests")
        }
...
mod ldk_test {
    ...
    impl NodeSigner for TestNodeSigner {
        ...
        fn sign_invoice( &self, _invoice: &RawBolt11Invoice, _recipient: Recipient,) -> Result<ecdsa::RecoverableSignature, ()> { unreachable!("unused by p2p tests") }
        fn sign_bolt12_invoice( &self, _invoice: &UnsignedBolt12Invoice,) -> Result<schnorr::Signature, ()> { unreachable!("unused by p2p tests") }
        fn sign_gossip_message( &self, _msg: UnsignedGossipMessage<'_>,) -> Result<ecdsa::Signature, ()> { unreachable!("unused by p2p tests") }
        fn get_expanded_key(&self) -> ExpandedKey { unreachable!("unused by p2p tests") }
        fn get_peer_storage_key(&self) -> PeerStorageKey { unreachable!("unused by p2p tests") }
        fn get_receive_auth_key(&self) -> ReceiveAuthKey { unreachable!("unused by p2p tests") }
        fn sign_message(&self, _msg: &[u8]) -> Result<String, ()> { unreachable!("unused by p2p tests") }
    }
