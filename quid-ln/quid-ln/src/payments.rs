...
impl PaymentWithMetadata<PaymentV2> {
    ...
    pub fn into_basic_payment(
        ...
        BasicPaymentV2 {
            ...
            fee,
            // channel_fee,
            partner_pk: self.payment.partner_pk(),
            partner_prop_fee: self.payment.partner_prop_fee(),
            partner_base_fee: self.payment.partner_base_fee(),
            status,
            status_str,
            address,
            invoice,
...
mod test {
    ...
    use quid_common::test_utils::{arbitrary, roundtrip};
    ...
    ///
    /// ```bash
    /// $ cargo test -p quid-ln --lib -- --ignored take_payments_snapshot --show-output
    /// ```
    #[ignore]
    #[test]
    ...
    ///
    /// ```bash
    /// $ cargo test -p quid-ln --lib -- take_basic_payment_v2_snapshot --ignored --nocapture
    /// ```
    /// NOTE: this lives here b/c `quid-common` can't depend on `quid-ln`.
