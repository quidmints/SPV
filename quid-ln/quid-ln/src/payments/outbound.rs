...
use quid_common::{ByteArray, ln::amount::Amount, time::TimestampMs};
...
/// Default max retry attempts for Quid-managed payment retries.
...
impl OutboundInvoicePaymentV2 {
    ...
    pub(crate) fn check_payment_failed(
        ...
        use OutboundInvoicePaymentStatus::*;
            ...
            matches!(id, PaymentId::Lightning(hash) if hash == self.hash),
...
pub enum LxOutboundPaymentFailure {
    ...
    InvalidOffer,
    /// API misuse error. Probably a bug in Quid code.
    QuidErr,
...
impl LxOutboundPaymentFailure {
    pub fn as_str(&self) -> &'static str {
        match self {
            ...
            Self::QuidErr => "probable bug in QUID user node payment router",
...
pub(crate) mod arbitrary_impl {
    ...
    impl Arbitrary for OutboundInvoicePaymentV2 {
        ...
        fn arbitrary_with(args: Self::Parameters) -> Self::Strategy {
            ...
            let preimage = any::<PaymentPreimage>().prop_map(move |preimage| {
            ...
            let kind = PaymentRail::Invoice.any_child_kind();
    ...
    impl Arbitrary for OutboundSpontaneousPaymentV2 {
        ...
        fn arbitrary_with(pending_only: Self::Parameters) -> Self::Strategy {
            ...
            let gen_osp = move |(
                ...
                let preimage: PaymentPreimage = preimage; // provides type hint
...
#[cfg(test)]
mod test {
    use quid_common::test_utils::roundtrip::json_unit_enum_backwards_compat;
    ...
    fn lx_outbound_payment_failure_json_backwards_compat() {
        let expected_ser = r#"["NoRetries","Rejected","Abandoned","Expired","NoRoute","MetadataTooLarge","UnknownFeatures","InvoiceRequestExpired","InvoiceRequestRejected","BlindedPathCreationFailed","InvalidOffer","QuidErr","Unknown"]"#;
