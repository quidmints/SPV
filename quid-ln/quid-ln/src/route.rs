...
use cfg_if::cfg_if;
use either::Either;
use quid_api::{cli::LspInfo, types::invoice::Invoice};
use quid_common::{
...
/// The default LDK [`Router`] impl with concrete Quid types filled in.
...
/// The Quid payment [`Router`] impl for both the Quid LSP and our user nodes.
///
...
impl QuidRouter {
    /// Create a new [`QuidRouter`] for user nodes.
    ...
    /// Create a new [`QuidRouter`] for LSP.
    pub fn new_lsp(
        network_graph: Arc<NetworkGraphType>,
        logger: TracingLogger,
        scorer: Arc<Mutex<ProbabilisticScorerType>>,
    ...
    fn default_router(&self) -> &DefaultRouterType {
        match self {
            QuidRouter::Node { default_router, .. } => default_router,
            QuidRouter::Lsp { default_router } => default_router,
        }
    }
}

impl Router for QuidRouter {
...
impl<'a> LastHopHint<'a> {
    ...
    pub fn new(
        ...
        channels.iter().fold(base, |acc, channel| {
            ...
            //   scerario where (1) Quid *isn't* subsidizing channel open costs
            ...
            // the largest HTLCs Quid's LSP is willing to forward to us. An
        ...
        let last_hop_hint = PaymentForwardNode {
            tlvs: ForwardTlvs {
                ...
                payment_relay: PaymentRelay {
                    ...
                    // TODO(phlip9): don't charge payer Quid fees, instead
    ...
    pub(crate) fn invoice_route_hints(&self) -> Vec<RouteHint> {
        let last_hop_hint = RouteHintHop {
            ...
            short_channel_id: self.intercept_scid.0,
            // TODO(phlip9): don't charge payer Quid fees, instead charge payee
            // via skimmed value.
            fees: RoutingFees {
...
/// Get a [`PaymentParameters`] from a payee or invoice in Quid's default way.
