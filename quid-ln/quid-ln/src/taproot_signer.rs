...
/// `(our_partial, our_pubnonce)`. The fleet holds BOTH funding halves, so it calls
/// this once per half (each with the OTHER half's pubnonce) and combines via the
/// existing [`aggregate_key_path_partials`]. Because the nonce is disjoint from both
