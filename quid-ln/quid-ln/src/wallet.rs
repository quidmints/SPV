...
//! [`RootSeed`]: quid_common::root_seed::RootSeed
...
#[cfg(test)]
use quid_common::ln::channel::LxOutPoint;
use quid_common::{
...
/// Derives BIP86 taproot (`tr()` key-path) output descriptors from a master xpriv.
///
/// Returns three descriptors (BIP389 multipath, external, internal) and the
/// account-level xpub at `m/86'/{coin}'/0'`. Consistent with `OnchainWallet::new`,
/// which builds the live wallet from BDK's `Bip86` template over the same path.
///
/// Where `coin` is 0 for mainnet, 1 for testnet/signet/regtest.
pub fn derive_bip86_descriptors(
    network: Network,
    ...
    // BIP86 (taproot key-path) account path: m/86h/{coin}h/0h. The whole wallet is
    // taproot so its external-0 destination (= the LP's coop-close/upfront-shutdown
    // script and its splice-funding inputs) is key-path P2TR, uniform with the taproot
    // channel funding output — BDK signs `tr()` key-path spends natively (no hand-roll).
    let account_path = [
        ChildNumber::from_hardened_idx(86).expect("Checked in tests"),
    ...
    // BIP389 multipath: tr([fp/86'/coin'/0']xpub/<0;1>/*)
    let multi_body = format!("tr({origin}{account_xpub}/<0;1>/*)");
    let multi_checksum =
        miniscript::descriptor::checksum::desc_checksum(&multi_body)
            .expect("valid descriptor");
    let multipath_descriptor = format!("{multi_body}#{multi_checksum}");

    // External: tr([fp/86'/coin'/0']xpub/0/*)
    let ext_body = format!("tr({origin}{account_xpub}/0/*)");
    let ext_checksum =
        miniscript::descriptor::checksum::desc_checksum(&ext_body)
            .expect("valid descriptor");
    let external_descriptor = format!("{ext_body}#{ext_checksum}");

    // Internal: tr([fp/86'/coin'/0']xpub/1/*)
    let int_body = format!("tr({origin}{account_xpub}/1/*)");
    let int_checksum =
...
impl OnchainWallet {
    ...
    /// [`RootSeed::derive_bip32_master_xprv`]: quid_common::root_seed::RootSeed::derive_bip32_master_xprv
    ...
    pub async fn init(
        ...
        let (wallet, wallet_created) = Self::new(
            master_xprv,
            network,
            fee_estimates,
            coin_selector,
    ...
    fn new(
        ...
        // Descriptor for external (receive) addresses: `m/86h/{0,1}h/0h/0/*` (BIP86
        // key-path taproot). The whole wallet is taproot so its external-0 destination
        // (= the LP's coop-close / upfront-shutdown script, and its splice-funding
        // inputs) is key-path P2TR — uniform with the taproot channel funding output,
        // and matching what the EVM records as `btcRecipientOf` (`0x5120||key`). BDK
        // signs `tr()` key-path spends natively (proven; no hand-roll).
        let external = Bip86(master_xprv, KeychainKind::External);
        // Descriptor for internal (change) addresses: `m/86h/{0,1}h/0h/1/*`
    ...
    pub(crate) fn dummy(
        ...
        let coin_selector = CoinSelector::default();
    ...
    /// Sign the wallet's inputs in `psbt` and extract the tx. Used by the LDK
    /// anchor `BumpTransactionEventHandler` (via `WalletSource::sign_psbt`) to
    /// CPFP-fund commitment / HTLC-claim txs — the replacement-cycling and
    /// flood-and-loot defense. We sign ONLY the wallet UTXO inputs the handler
    /// added for fees; the handler applies the channel (anchor/HTLC) input's
    /// witness AFTER this returns. `extract_tx` tolerates that still-unsigned
    /// channel input (it gets an empty witness, which the handler overwrites);
    /// it only requires a value on every input, which the handler provides as a
    /// `witness_utxo`.
    pub fn sign_psbt(
        &self,
        ...
        let sign_opts = bdk_wallet::SignOptions {
            // REQUIRED: the channel input carries only a `witness_utxo`; without
            // trusting it, BDK's sign errors on that non-finalized input.
            trust_witness_utxo: true,
            ..Default::default()
        };
        // `finalized=false` is expected — only the wallet's own inputs are ours.
        self.write()
            .sign(&mut psbt, sign_opts)
            .context("Failed to sign anchor psbt")?;
        let tx = psbt
            .extract_tx()
            .context("Failed to extract signed anchor tx")?;
        Ok(tx)
    }

    /// Sign ONLY the inputs we own in an interactive-funding / splice transaction,
    /// leaving the rest (the channel's shared 2-of-2 input, the counterparty's
    /// inputs) untouched, and return the partially-signed tx for
    /// `ChannelManager::funding_transaction_signed`. Differs from [`sign_psbt`] in
    /// two ways the splice co-sign requires: (1) the event hands us a BARE unsigned
    /// tx, so we populate each owned input's `witness_utxo` from the wallet's UTXO
    /// set first (BDK can't sign a segwit input without it); (2) we extract WITHOUT
    /// the fee check, because the PSBT is intentionally PARTIAL — the shared channel
    /// input + the counterparty's inputs legitimately lack value info here (LDK
    /// supplies the channel signature separately via the commitment/tx_signatures
    /// flow). Mirrors ldk-node's funding-tx signer.
    pub fn sign_interactive_funding(
        &self,
        unsigned: bitcoin::Transaction,
    ) -> anyhow::Result<bitcoin::Transaction> {
        let mut psbt = bitcoin::psbt::Psbt::from_unsigned_tx(unsigned)
            .context("PSBT from unsigned interactive-funding tx")?;
        let utxos = self.get_utxos();
        for (i, txin) in psbt.unsigned_tx.input.iter().enumerate() {
            if let Some(u) = utxos.iter().find(|u| u.outpoint == txin.previous_output) {
        ...
        // `finalized=false` is expected: only our own inputs are ours to sign.
        self.write()
            .sign(&mut psbt, sign_opts)
            .context("Failed to sign interactive-funding psbt")?;
        // SAFETY of the unchecked-fee extract: this is NOT a fee-blind
        // signature. The signer above signs ONLY inputs the wallet owns — we
        // populated `witness_utxo` solely for outpoints found in our own UTXO set,
        // and BDK refuses to sign a segwit input lacking that value info, so a
        // shared/counterparty input simply isn't signed by us. Independently, LDK
        // VALIDATES the interactive-funding transaction's structure and feerate
        // during the channel/splice negotiation BEFORE it emits
        // `FundingTransactionReadyForSigning`; the PSBT handed here is therefore
        // already fee-sane. `extract_tx_unchecked_fee_rate` is REQUIRED (not a
        // shortcut) precisely because the partial PSBT's shared 2-of-2 input + the
        // counterparty's inputs legitimately carry no value info at this point, so
        // a fee-checking extract would spuriously reject a well-formed tx. The LP
        // legitimately contributes inputs to a splice it initiates and runs this
        // same path, so asserting "zero owned inputs" would be wrong — the owned-
        // inputs-only signing + LDK's pre-emit validation are the real invariants.
    ...
    pub fn get_balance(&self) -> OnchainBalance {
        ...
        // Convert bdk_wallet::Balance to quid_common::ln::balance::Balance.
        // Not using a From impl bc we don't want `quid-common` to depend on
        // BDK.
    ...
    fn unconfirmed_transaction_evicted_at(
        &self,
        evicted_at: TimestampMs,
        txid: quid_common::ln::hashes::Txid,
    ) {
        let evicted_at_secs = evicted_at.to_duration().as_secs();
        self.inner
            .write()
...
pub fn spawn_wallet_persister_task<PS: QuidPersister>(
    persister: PS,
...
async fn do_wallet_persist<PS: QuidPersister>(
    persister: &PS,
    wallet: &OnchainWallet,
    filename: &'static str,
) {
...
impl CoinSelectionAlgorithm for CoinSelector {
...
fn fake_drain_script() -> bitcoin::ScriptBuf {
    // 34-byte witness-v1 P2TR (`OP_1 <32-byte x-only key>`) matching the BIP86 wallet's
    // key-path change/drain output, so drain/sweep fee estimation uses the right output
    // weight (was a 22-byte P2WPKH placeholder from the pre-taproot wallet).
...
mod arbitrary_impl {
    ...
    use quid_common::{root_seed::RootSeed, test_utils::arbitrary};
    use proptest::{
        arbitrary::any,
        option,
    ...
    pub(super) fn any_changeset() -> impl Strategy<Value = ChangeSet> {
        ...
        let just_descriptor = Just({
            let (descriptor, _, _) = Bip86(master_xprv, KeychainKind::External)
                .build(network.to_bitcoin())
                .unwrap();
            descriptor
        });
        let just_change_descriptor = Just({
            let (descriptor, _, _) = Bip86(master_xprv, KeychainKind::Internal)
                .build(network.to_bitcoin())
                .unwrap();
            descriptor
        });
...
mod test {
    ...
    use quid_api::types::payments::ClientPaymentId;
    use quid_common::{
        ln::hashes::Txid,
        root_seed::RootSeed,
        sat,
    ...
    fn preflight_doesnt_persist() {
        ...
        h.assert_no_persists_in(|h| {
            ...
            assert_eq!(fee.high.map(|x| x.amount.sats_u64()), Some(387));
            assert_eq!(fee.normal.amount.sats_u64(), 309);
            assert_eq!(fee.background.amount.sats_u64(), 187);
        });
        ...
        h.assert_no_persists_in(|h| {
            ...
            let fee = h.wallet.preflight_pay_onchain(req, h.network).unwrap();
            assert_eq!(fee.high.map(|x| x.amount.sats_u64()), Some(532));
            assert_eq!(fee.normal.amount.sats_u64(), 425);
            assert_eq!(fee.background.amount.sats_u64(), 257);
        });
    }
    ...
    fn test_sweep_tx() {
        ...
        // With BIP86 P2TR key-path inputs the sweep tx is cheaper than the old
        // P2WPKH wallet, so 410 sats now nets a below-dust P2WPKH output
        // (410 - 121 fee = 289 < 294 dust limit) and the sweep is rejected.
        h.ww().fund(KeychainKind::External, sat!(410));
        ...
        // Conservation invariant: swept output + fee == total inputs
        // (410 dust utxo + 5_000 utxo = 5_410).
        assert_eq!(amount_sats + fee.sats_u64(), 5_410);
    ...
    /// ```bash
    /// $ cargo test -p quid-ln --lib -- --ignored test_changeset_internal_snapshot --show-output
    /// ```
    #[ignore]
    #[test]
    ...
    ///
    /// ```bash
    /// $ cargo test -p quid-ln --lib -- --ignored dump_proptest_wallet_snapshot --show-output
    /// ```
    #[ignore]
    #[test]
    ...
    ///
    /// ```bash
    /// $ cargo test -p quid-ln --lib -- --ignored dump_wallet_snapshot --show-output
    /// ```
    #[ignore]
    #[test]
    ...
    /// Test that our `derive_bip86_descriptors` produces descriptors matching
    /// BDK's `Bip86` template, and that our account-level xpub can derive
    /// addresses matching both descriptors.
    #[test]
    fn derive_bip86_descriptors_matches_bdk() {
        ...
        proptest!(cfg, |(
            ...
            let OnchainDescriptors {
                ...
                account_xpub,
            } = derive_bip86_descriptors(network, master_xprv);

            // BDK's implementation.
            // This is also how we use `Bip86` in `OnchainWallet::new`
            ...
            let ext_xpub = account_xpub
                ...
                .unwrap();
            // BIP86 taproot key-path: address is P2TR over the BIP341-tweaked
            // internal key with an empty script tree (no merkle root), exactly
            // what BDK's `tr(KEY)` descriptor produces.
            let (ext_xonly, _) = ext_xpub.public_key.x_only_public_key();
            let our_ext_addr = bitcoin::Address::p2tr(
                &SECP256K1,
                ext_xonly,
                None,
                network.to_bitcoin(),
            ...
            let int_xpub = account_xpub
                ...
                .unwrap();
            let (int_xonly, _) = int_xpub.public_key.x_only_public_key();
            let our_int_addr = bitcoin::Address::p2tr(
                &SECP256K1,
                int_xonly,
                None,
                network.to_bitcoin(),
