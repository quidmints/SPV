//! Quid TLS configs, certs, and utilities.
...
pub mod ed25519_ext;
/// Certs and utilities related to Quid's CA.
pub mod quid_ca;
...
/// Re-export all of `quid_tls_core`.
pub use quid_tls_core::*;
...
pub static DEFAULT_SUBJECT_ALT_NAMES: LazyLock<Vec<rcgen::SanType>> =
    LazyLock::new(|| {
        vec![rcgen::SanType::DnsName(
            Ia5String::from_str("quid.io").unwrap(),
        )]
    });

/// Build an [`rcgen::CertificateParams`] with Quid presets and optional
/// overrides.
...
pub fn build_rcgen_cert_params(
    ...
    params.distinguished_name = quid_distinguished_name(common_name);
...
/// Build a Quid Distinguished Name given a Common Name.
pub fn quid_distinguished_name(common_name: &str) -> DistinguishedName {
    let mut name = DistinguishedName::new();
    name.push(DnType::CountryName, "US");
    name.push(DnType::StateOrProvinceName, "CA");
    name.push(DnType::OrganizationName, "quid-app");
