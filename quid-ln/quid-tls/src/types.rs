...
pub struct CertWithKey {
    ...
    ///
    /// # Root Quid CA key rotation
    ///
    /// 99% of the time, "Quid CA" TLS does not require an intermediate cert
    /// chain.
    ///
    /// Only if this corresponds to an end-entity cert used to authenticate
    /// ourselves for "Quid CA" TLS, and we're in the midst of a Root Quid CA
    /// key rotation, is an intermediate cert chain required, in which case it
    /// should contain the cert of the NEW Quid CA (signed by the old Quid CA).
    ///
    /// See the docs on `QuidRootCaCert` for more info.
