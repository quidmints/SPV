#!/usr/bin/env node
// Measure α: the exponent in range-value ∝ p^α for QU!D's ±2% RESEAT-following concentrated range
// (arbed within the range per the correction — so it realizes the concentrated IL). Then L=1/α is the
// leverage that cancels the IL. α=0.5 = 50/50 full-range; a concentrated reseat range may differ.
const fs=require('fs');
...
// reseat-following range: value trajectory over a normalized path (p0=1, value0=1)
function rangeValue(Pin){
...
{ const {P,vals}=rangeValue(up); const a=fitAlpha(P,vals);
  console.log(`CLEAN +smooth path:  α = ${a.toFixed(3)}   ⇒  L=1/α = ${(1/a).toFixed(2)}   (final range value ${vals[vals.length-1].toFixed(3)} vs HODL ${P[P.length-1].toFixed(2)})`); }
...
for(const [name,file] of [['ETH','eth.json'],['BTC','btc.json']]){
  ...
  const out=[90,180].map(LEN=>{
    const as=[]; for(let s=0;s+LEN<Pf.length;s+=14){const {P,vals}=rangeValue(Pf.slice(s,s+LEN+1));const a=fitAlpha(P,vals);if(isFinite(a))as.push(a);}
...
console.log('\nREAD: if α≈0.5, L≈2 (YB default holds for QU!D). If the concentrated reseat range gives α≠0.5,');
