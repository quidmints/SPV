...
// loses (L²−L)/2 · σ² per year vs L× the underlying. The YB overlay levers the BAND (σ_band ≈ α·σ_ETH).
...
//                     − (L²−L)/2·σ_band² (variance drag)
//   break-even TO: L·FEE·TO = (L−1)·carry + (L²−L)/2·σ_band²
const FEE=0.0005, CARRY=0.07;
function breakevenTO(L, sigBand){ return ((L-1)*CARRY + (L*L-L)/2*sigBand*sigBand) / (L*FEE); }
console.log('ANALYTICAL break-even turnover for ETH leverage (variance-drag closed form):\n');
console.log('  L     σ_band   carry-only  +drag    TOTAL break-even TO');
...
console.log('\nα≈0.5 ⇒ σ_band ≈ 0.5·σ_ETH. ETH σ~0.6-0.9 ⇒ σ_band~0.30-0.45.');
...
console.log('   was wrong because it omitted the +1000·σ_band² (≈ +90-250x) drag term.');
console.log('');
...
console.log('AND the sim to flip it. For BTC (σ_band~0.2-0.25): break-even ~110-150x — closer, hence the');
