#!/usr/bin/env node
// GATE SIM: does a CONSTANT-LTV overlay on a (reseat-following) range share cancel the range IL and
// track ∝ p (= true YB), and how does REBALANCE GRANULARITY affect fidelity? Analytic core:
//   dE/E = L·(dB/B) − carry.  A reseat-following AMM range has B ∝ p^α (α≈0.5 for 50/50). So the
...
const rangeVal=(p,alpha)=>Math.pow(p,alpha);   // reseat-following range value, relative to p0=1
...
function overlay(Pin,L,R,alpha){
  ...
  let coll = L*E; debt = (L-1)*E; units = coll/rangeVal(P[0],alpha);  // collateral in range units
  let liq=false, carryPaid=0;
  for(let i=1;i<P.length;i++){
    coll = units*rangeVal(P[i],alpha);
    const dCarry = BORROW*debt*DT; debt += dCarry; carryPaid += dCarry;
    E = coll - debt;
    if(E<=0 || coll/debt < MCR){ liq=true; break; }
    if(i%R===0){ coll=L*E; const newDebt=(L-1)*E; debt=newDebt; units=coll/rangeVal(P[i],alpha); }
  }
  if(liq) return {liq:true};
  const pe=P[P.length-1]; coll=units*rangeVal(pe,alpha); E=coll-debt;
  return {liq:false, E, hodl:pe, range:rangeVal(pe,alpha), carryPaid, trackErr:(E-pe)/pe};
}

console.log('Overlay equity vs HODL-ETH(∝p) and unlevered range(∝p^α). α=0.5 (50/50 range) ⇒ L=2 should track ∝p.\n');
...
console.log('  L     R(steps)   equity   HODL(p)   range(√p)   track-err-vs-HODL');
for(const L of [2.0]) for(const R of [1,10,40]){
  const o=overlay(up,L,R,0.5);
  if(o.liq){console.log(`  ${L}    ${R}        LIQUIDATED`); continue;}
  console.log(`  ${L.toFixed(1)}   ${String(R).padEnd(10)} ${o.E.toFixed(3).padEnd(9)}${o.hodl.toFixed(3).padEnd(10)}${o.range.toFixed(3).padEnd(11)}${(o.trackErr*100).toFixed(1)}%`);
}
console.log('  (track-err→0 as R→1 ⇒ continuous constant-LTV-2x of a √p range = ∝p = zero IL. Coarse R = residual curvature.)\n');
...
console.log('\nREAD: small track-err with no-liq ⇒ the range HOSTS true YB (overlay = LEVAMM). Where liq% is high,');
console.log('that is the L=2 tail (down-moves); the user wanted LIGHT leverage for the IL-offset (partial track, ~0 liq) and');
console.log('full L=2 only for those who accept the tail. The α=0.5 assumption (50/50 range) is the calibration knob:');
console.log('measure QU!D range α from real reseat behavior to set the exact L (L=1/α) — that is the one real follow-up.');
