...
export interface Contracts {
  ...
  range:       string  // ETH range manager (BTC side is `vault`, not a flag on this one)
  // §E235-spa — TWO ENGINES, ONE ACCESSOR EACH. The isBTC split replaced one `Core` carrying
  // `POOLED_ETH`/`POOLED_BTC`/`POOLED_USD_ETH`/`POOLED_USD_BTC` with two instances that each expose
  // one `POOLED()`/`POOLED_USD()`. So the range is chosen by ADDRESS now, not by a suffix or a bool.
  // ⚠️ The dangerous version of this fix is renaming the reads and leaving ONE address: every BTC
  // read would then answer off the ETH engine — a live call, a plausible number, the wrong range.
  // Two fields make that mistake impossible to write by accident.
  rangeCore:   string  // ETH engine — POOLED, POOLED_USD, observe, swapUsdBtc
  rangeCoreBtc: string // BTC engine — the SAME accessors on the WBTC/btcRisk instance
  // §E235-spa — the BTC RANGE MANAGER, which the client never had an address for. Its BTC reads
  // used to work by calling BTC-suffixed accessors on `range`, because one contract carried both
  // ranges' state. `Vault` now holds the BTC range (autoManaged, lpShares, selfManaged, positions),
  // so those reads need ITS address. Unlike `btcCore` this one IS already in the deploy record.
  vault:       string  // BTC range manager (Vault.sol) — BTC-side LP state + BTC self-managed
...
import l1raw from '../../../evm/deployments/l1.json'

// ⚠️ READ THE RECORD AS A LOOSE MAP, DELIBERATELY, AND THE REASON IS NOT CONVENIENCE.
// `deployments/l1.json` is a committed ARTIFACT that only `DeployL1_s` regenerates, so TypeScript
// infers its type from WHATEVER LAST DEPLOY WROTE IT — which makes every key the script has since
// added or renamed a COMPILE ERROR in this file rather than a runtime zero. Measured 2026-08-28: the
// script writes `range` and `btcCore` (`DeployL1_s.sol:466`, `:477`) and the committed record
// carries neither, because it predates both and still keys the ETH range manager as `vogue`.
// ⇒ The code here is CORRECT and the artifact is BEHIND. Widening the type is what lets the two
// disagree safely: an absent key yields ZERO through `dep()`, and a ZERO address fails LOUDLY
// (`eth_call` to 0x0 returns '0x' and the decode throws) instead of quietly answering with some
// other contract's numbers. That is the same handling `btcCore` already carried, applied to the
// whole record instead of one cast at one line.
// ⛔ DO NOT "FIX" THIS BY HAND-EDITING l1.json. CREATE addresses are f(deployer, nonce), so an
// invented one is a fabricated address that silently answers nothing. RE-RUN THE DEPLOY.
const l1 = l1raw as Record<string, string | number | undefined>
const addr = (v: string | number | undefined): string | undefined =>
...
export const CONTRACTS: Contracts = {
  basket:      ok(process.env.NEXT_PUBLIC_BASKET,       dep(addr(l1.basket))),
  aux:         ok(process.env.NEXT_PUBLIC_AUX,          dep(addr(l1.aux))),
  range:       ok(process.env.NEXT_PUBLIC_RANGE,        dep(addr(l1.range))),
  rangeCore:   ok(process.env.NEXT_PUBLIC_RANGE_CORE,   dep(addr(l1.core))),
  // ⚠️ `btcCore` IS NOT IN THE COMMITTED `l1.json` YET — `DeployL1_s` only started writing it in the
  // same change as this line, and that record is REGENERATED BY A DEPLOY RUN, not editable by hand
  // (CREATE addresses are f(deployer, nonce), so inventing one would be a fabricated address that
  // silently answers nothing). Hence the cast: the key is absent today, `dep()` yields ZERO, and the
  // BTC reads fail LOUDLY (eth_call to 0x0 returns '0x' and decodeFunctionResult throws) instead of
  // quietly reporting the ETH range's numbers. ⇒ RE-RUN THE DEPLOY TO POPULATE IT — until then the
  // BTC panels are down BY CONSTRUCTION, which is the honest state and is visible on first load.
  rangeCoreBtc: ok(process.env.NEXT_PUBLIC_RANGE_CORE_BTC,
                   dep(addr(l1.btcCore))),
  vault:       ok(process.env.NEXT_PUBLIC_VAULT,        dep(addr(l1.vault))),
  btcChannels: ok(process.env.NEXT_PUBLIC_BTC_CHANNELS, dep(addr(l1.btcChannels))),
