...
export interface Contracts {
  ...
  band:       string  // ETH band manager (BTC side is `vault`, not a flag on this one)
  ...
  // one `POOLED()`/`POOLED_USD()`. So the band is chosen by ADDRESS now, not by a suffix or a bool.
  // ⚠️ The dangerous version of this fix is renaming the reads and leaving ONE address: every BTC
  // read would then answer off the ETH engine — a live call, a plausible number, the wrong band.
  // Two fields make that mistake impossible to write by accident.
  bandCore:   string  // ETH engine — POOLED, POOLED_USD, observe, swapUsdBtc
  bandCoreBtc: string // BTC engine — the SAME accessors on the WBTC/btcRisk instance
  // §E235-spa — the BTC BAND MANAGER, which the client never had an address for. Its BTC reads
  // used to work by calling BTC-suffixed accessors on `band`, because one contract carried both
  // bands' state. `Vault` now holds the BTC band (autoManaged, lpShares, selfManaged, positions),
  // so those reads need ITS address. Unlike `btcCore` this one IS already in the deploy record.
  vault:       string  // BTC band manager (Vault.sol) — BTC-side LP state + BTC self-managed
...
export const CONTRACTS: Contracts = {
  ...
  band:       ok(process.env.NEXT_PUBLIC_BAND,        dep(l1.band)),
  bandCore:   ok(process.env.NEXT_PUBLIC_BAND_CORE,   dep(l1.core)),
  ...
  // quietly reporting the ETH band's numbers. ⇒ RE-RUN THE DEPLOY TO POPULATE IT — until then the
  // BTC panels are down BY CONSTRUCTION, which is the honest state and is visible on first load.
  bandCoreBtc: ok(process.env.NEXT_PUBLIC_BAND_CORE_BTC,
