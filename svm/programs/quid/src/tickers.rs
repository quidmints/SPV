//! Ticker tables — Pyth feed ids and their price accounts.
//!
//! Lifted wholesale out of `etc.rs`, which was 4,429 lines of which 2,655 were
//! these maps: data, not logic, and it made the risk engine hard to read for no
//! reason. Nothing here changed in the move.
//!
//! The `slim` feature builds only the tickers that can actually be delivered —
//! see `tickers_slim.rs` for why that matters and what it costs.
